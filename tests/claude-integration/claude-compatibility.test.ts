...
describe("Claude routed compatibility foundation", () => {
  test("collects stable feature codes without reading credentials", () => {
    ...
    ).toEqual(["beta_context_1m_2025_08_07", "web_search_tool"]);
  });

  test("fails closed for unmappable routed content while native Anthropic stays allowed", () => {
    const body = {
      messages: [{ role: "user", content: [{ type: "document", title: "x" }] }],
    };
    expect(
      analyzeClaudeCompatibility(body, {
        mode: "enforce",
        adapter: "openai-chat",
      }).decision,
    ).toBe("reject");
    ...
    ).toMatchObject({ compatible: true, decision: "allow" });
  });
  ...
  test("recognizes deferred tools by activation value, not field presence", () => {
    expect(
      collectClaudeFeatureCodes({ defer_tools: false, deferred_tools: [] }),
    ).not.toContain("deferred_tools");
    expect(
      collectClaudeFeatureCodes({
        tools: [{ type: "function", name: "later", defer_loading: true }],
      }),
    ).toContain("deferred_tools");
    expect(collectClaudeFeatureCodes({ deferred_tools: ["later"] })).toContain(
      "deferred_tools",
    );
  });
  ...
  test("shadow mode reports, but does not reject, an incompatible feature", () => {
    const result = analyzeClaudeCompatibility(
      { context_management: { edits: [{ op: "remove" }] } },
      { mode: "shadow", adapter: "openai-chat" },
    );
  ...
  test("feature evidence is bounded", () => {
    const betas = Array.from(
      { length: 40 },
      (_, index) => `feature-${index}-${"x".repeat(100)}`,
    ).join(",");
  ...
  test("the Messages endpoint gate is opt-in, fail-closed, and reason-bounded", () => {
    const body = {
      messages: [
        { role: "user", content: [{ type: "document", title: "secret-name" }] },
      ],
    };
    expect(() =>
      enforceClaudeCompatibility(body, { adapter: "openai-chat" }),
    ).not.toThrow();
    expect(() =>
      enforceClaudeCompatibility(body, {
        mode: "shadow",
        adapter: "openai-chat",
      }),
    ).not.toThrow();
