 ...
 * became the thirteenth retained store the very next day, and three more state-store
 * registrations landed after that, so all eight README files have advertised 12 and 24 ever
 * since. Review does not catch this: an English diff of one number looks complete on its own,
 ...
 * it sum to 45, but the page also lists stores belonging to no roster, and its last bullet
