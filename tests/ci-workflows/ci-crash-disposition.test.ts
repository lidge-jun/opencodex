...
// Sources the real classifier once and prints one verdict per (status, log) pair it is given.
// One process for the whole table, not one per case: the classifier is a pure function of its
// two arguments, so a process per case buys no isolation and costs a subprocess teardown each.
const PROBE = [
  "set -euo pipefail",
  'source "$1"',
  "shift",
  'while [ "$#" -gt 0 ]; do',
  '  if is_bun_runtime_crash "$1" "$2"; then echo CRASH; else echo TEST; fi',
  "  shift 2",
  "done",
].join("\n");
...
function classifyAll(pairs: ReadonlyArray<readonly [number, string]>): string[] {
  const directory = mkdtempSync(join(tmpdir(), "ocx-crash-classifier-"));
  try {
    const args: string[] = [];
    pairs.forEach(([status, log], index) => {
      const logPath = join(directory, `bun.${index}.log`);
      writeFileSync(logPath, log, "utf8");
      args.push(String(status), logPath);
    });
    const result = Bun.spawnSync(
      ["bash", "--noprofile", "--norc", "-c", PROBE, "probe", CLASSIFIER, ...args],
    ...
    const verdicts = decode(result.stdout).trim().split("\n");
    expect(verdicts).toHaveLength(pairs.length);
    return verdicts;
  } finally {
...
describe.skipIf(process.platform === "win32")("the shared Bun crash classifier, executed", () => {
  ...
  // Every case is evaluated in one shell process before the assertions run, so the table below
  // still names and reports each case individually.
  let verdicts: string[] = [];
  beforeAll(() => {
    verdicts = classifyAll(cases.map(([, status, log]) => [status, log] as const));
  }, SPAWN_BUDGET_MS);

  test.each(cases.map((entry, index) => [...entry, index] as const))(
    "%s",
    (_name, _status, _log, expected, index) => {
      expect(verdicts[index]).toBe(expected);
    },
  );
});
