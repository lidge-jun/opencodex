...
describe("CLI subcommand help", () => {
  ...
  test("help command routes to subcommand help", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stderr).toBe("");
    expect(result.stdout).toContain("Usage: ocx start [--port <port>] [--socks5 [host:port]]");
  ...
  test("start rejects unknown and partially numeric port arguments", () => {
    const cases = [
      ...
      { args: ["start", "--bad"], expected: "Usage: ocx start [--port <port>] [--socks5 [host:port]]" },
      { args: ["start", "--port", "1234", "--extra"], expected: "Usage: ocx start [--port <port>] [--socks5 [host:port]]" },
    ];
