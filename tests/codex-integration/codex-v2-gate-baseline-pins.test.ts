/**
 * Pristine-baseline multi-agent pin cases, held in a sibling file.
 *
 * Split out of codex-v2-gate.test.ts for the reason recorded in d3ca5522db and
 * #4908: that file sits at its file-size ratchet cap, the cap only ever moves
 * downward, and a case added after the cap was set fails the ratchet for every
 * later pull request rather than only its own. The cases are unchanged.
 */
...
describe("3-state multi-agent mode: pristine baseline pins", () => {
