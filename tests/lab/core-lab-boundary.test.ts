...
describe("activation window stays synchronous", () => {
  ...
  const SYNC_WINDOW_RECEIVER_CALLS: Record<string, string> = {
    ...
    "optionalListeners.start()": "Instance method on OptionalListenerSet. Declared `(ctx): void`; it binds the optional link listener synchronously and starts the existing Claude intercept and the opt-in ChatGPT send-unblock lifecycles, both fire-and-forget: each stores its start promise, never awaits it, and turns every failure into a warning. Its stop() runs inside the async stop wrapper, which this scan skips.",
