...
describe("Opper gateway provider", () => {
  ...
  test("validates keys against the authenticated compat model list", () => {
    // /v3/compat/models answers 401 without a key, so the default discovery URL is a real
    // key check and the preset must not opt into `apiKeyValidation: "unknown"`.
