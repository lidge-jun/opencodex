...
describe("codex-auth API", () => {
  ...
  test("main reset-credit consume owns native main through destructive upstream work and rejects post-fence work", async () => {
    ...
    const request = () => {
      const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: resetCreditConsumeBody(MAIN_CODEX_ACCOUNT_ID),
      });
      return handleResetCreditConsume(req, makeConfig());
    };
  ...
  for (const principal of [undefined, "admin-token", "gui-session"] as const) {
    test(`reset-credit consume refuses ${principal ?? "missing"} consent authority before upstream work`, async () => {
      ...
      const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          accountId: "pool-consent-boundary",
          operationId: randomUUID(),
          confirmed: true,
        }),
      });

      const resp = principal === undefined
        ? await handleCodexAuthAPI(req, new URL(req.url), makeConfig())
        : await handleResetCreditConsume(req, makeConfig(), principal);
  ...
  test("reset-credit consume reads the capability-bound account and operation from a bodyless request", async () => {
    const config = makeConfig();
    const accountId = "pool-local-consent";
    const operationId = randomUUID();
    seedPoolAccount(config, { id: accountId, email: "local-consent@example.test" });
    globalThis.fetch = (async (input: RequestInfo | URL) => {
      expect(String(input)).toContain("/rate-limit-reset-credits/consume");
      return Response.json({ code: "nothing_to_reset" });
    }) as typeof fetch;
    const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
      method: "POST",
    ...
    const resp = await handleResetCreditConsume(req, config, "local-reset-credit-capability");

    expect(resp?.status).toBe(200);
    expect(await resp?.json()).toEqual({ code: "nothing_to_reset" });
  });

  test("reset-credit consume rejects non-canonical uppercase operation ids", async () => {
    const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
        accountId: "pool-uppercase-operation",
  ...
  test("reset-credit consume requires a caller-stable operation id", async () => {
    const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ accountId: "pool-without-operation" }),
    });
    const resp = await handleResetCreditConsume(req, makeConfig());
    expect(resp!.status).toBe(400);
    expect(await resp!.json()).toEqual({
  ...
  test("reset-credit consume sanitizes a pre-dispatch client abort", async () => {
    ...
    try {
      const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: resetCreditConsumeBody("pool-aborted"),
        signal: controller.signal,
      });
  ...
  test("reset-credit consume resumes an ambiguous operation id and terminally short-circuits it", async () => {
    const config = makeConfig();
    seedPoolAccount(config, { id: "pool-ambiguous", email: "ambiguous@example.test" });
    const operationId = randomUUID();
    ...
    try {
      globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes("/backend-api/wham/rate-limit-reset-credits/consume")) {
          ...
          seenOperationIds.push(body.redeem_request_id ?? "");
      ...
      const request = () => {
        const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ accountId: "pool-ambiguous", operationId }),
        });
      ...
      expect(await first?.json()).toEqual({ error: "Invalid upstream reset-credit response" });
  ...
  test("reset-credit consume resumes the prior id when the client starts a new retry intent", async () => {
    const config = makeConfig();
    seedPoolAccount(config, { id: "pool-conflict", email: "conflict@example.test" });
    let consumeCalls = 0;
    const seenOperationIds: string[] = [];
    const originalFetch = globalThis.fetch;
    try {
      ...
      const call = async (operationId: string) => {
        const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ accountId: "pool-conflict", operationId }),
        });
  ...
  test("concurrent reset-credit retries share one process-local consume flight", async () => {
    const config = makeConfig();
    seedPoolAccount(config, { id: "pool-flight", email: "flight@example.test" });
    const operationId = randomUUID();
    let consumeCalls = 0;
    let releaseConsume!: () => void;
    ...
    try {
      globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (url.includes("/backend-api/wham/rate-limit-reset-credits/consume")) {
          consumeCalls += 1;
          expect((JSON.parse(String(init?.body)) as { redeem_request_id: string }).redeem_request_id)
            .toBe(operationId);
          consumeStarted();
      ...
      const request = () => {
        const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({ accountId: "pool-flight", operationId }),
        });
        return handleResetCreditConsume(req, config);
      };
      const first = request();
  ...
  test("reset-credit consume returns its terminal code without waiting for quota refresh", async () => {
    ...
    try {
      ...
      const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: resetCreditConsumeBody("pool-reset"),
      });
      const resp = await handleResetCreditConsume(req, config);
      expect(resp!.status).toBe(200);
  ...
  test("reset-credit already_redeemed returns immediately and never invents a local decrement", async () => {
    ...
    try {
      ...
      const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: resetCreditConsumeBody("pool-idempotent"),
      });
      const resp = await handleResetCreditConsume(req, config);
      expect(resp!.status).toBe(200);
  ...
  test("reset-credit consume omits remaining when refreshed quota has no credit count", async () => {
    ...
    try {
      ...
      const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: resetCreditConsumeBody("pool-nocount"),
      });
      const resp = await handleResetCreditConsume(req, config);
      expect(resp!.status).toBe(200);
      expect(await resp!.json()).toEqual({ code: "reset" });
  ...
  test("reset-credit consume omits remaining when pool WHAM refresh is non-2xx", async () => {
    ...
    try {
      ...
      const req = new Request("http://localhost/api/codex-auth/reset-credits/consume", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: resetCreditConsumeBody("pool-wham-fail"),
      });
      const resp = await handleResetCreditConsume(req, config);
      expect(resp!.status).toBe(200);
      expect(await resp!.json()).toEqual({ code: "reset" });
