...
export const GUARDS: Guard[] = [
  ...
  },
  {
    ...
    file: "src/server/audio-speech.ts",
    from: "  if (unknown) return invalid(`Unsupported speech field: ${unknown.slice(0, 64)}`);",
    to: "",
  ...
  {
    name: "the speech reserved name is refused where it is written",
    file: "src/config/voice-target.ts",
    from: '    if (resolution.kind === "openai" && route.reservedUnservable) {',
    to: "    if (false) {",
    expect: "the reserved name is refused for speech, which has no built-in backend",
    suite: VOICE,
  },
  {
    name: "a voice endpoint with embedded credentials is refused",
    file: "src/config/voice-target.ts",
    from: "  if (urlError) return `${label} ${urlError}`;",
    to: "",
    expect: "an endpoint carrying embedded credentials is refused, naming the field",
    suite: VOICE,
  },
  {
    name: "a non-numeric or out-of-range speed is refused",
    file: "src/server/audio-speech.ts",
    from: '  if (body.speed !== undefined\n    && (typeof body.speed !== "number" || !Number.isFinite(body.speed) || body.speed < 0.25 || body.speed > 4)) {',
    to: "  if (false) {",
    expect: "refuses a speed that is not a number",
    suite: VOICE,
  },
  {
    name: "non-string instructions are refused",
    file: "src/server/audio-speech.ts",
    from: '  if (body.instructions !== undefined && typeof body.instructions !== "string") {',
    to: "  if (false) {",
    expect: "refuses instructions that are not a string",
    suite: VOICE,
  },
  {
    name: "a declared content-length past 32 MiB is refused before reading",
    file: "src/server/audio-transcriptions.ts",
    from: "  if (length && /^\\d+$/.test(length) && Number(length) > AUDIO_BODY_MAX_BYTES) {",
    to: "  if (false) {",
    expect: "refuses a content-length past 32 MiB",
    suite: VOICE,
  },
  {
    name: "a file past the 25,000,000-byte cap is refused",
    file: "src/server/audio-transcriptions.ts",
    from: '  if (file.size > AUDIO_FILE_MAX_BYTES) return invalid("Audio file exceeds 25,000,000 bytes", 413);',
    to: "",
    expect: "refuses a file past 25,000,000 bytes",
    suite: VOICE,
  },
  {
    name: "a response_format outside json/text is refused",
    file: "src/server/audio-transcriptions.ts",
    from: '  if (format !== "json" && format !== "text") return invalid("response_format must be json or text");',
    to: "",
    expect: "refuses a response_format that is neither json nor text",
    suite: VOICE,
  },
  {
  ...
  },
  {
    ...
    file: "src/server/audio-transcriptions.ts",
    from: '    if (name !== "file" && (typeof value !== "string" || Buffer.byteLength(value) > AUDIO_FIELD_MAX_BYTES)) {',
    to: "    if (false) {",
