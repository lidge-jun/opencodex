...
test("the management convergence boundary strips unexpected private disposition fields", async () => {
  ...
  const response = await handleManagementAPI(request, new URL(request.url), live, {
    saveConfigPreservingClaudeCode: () => {},
    createManagementConvergeCodex: () => async () => projectCatalogOnlyOutcome({
      changed: false,
  ...
  expect(JSON.stringify(payload)).not.toContain("acct-private");
  expect(JSON.stringify(payload)).not.toContain("/private/catalog/path");
});

test("the route inventory contains exactly the specified management convergence calls", () => {
