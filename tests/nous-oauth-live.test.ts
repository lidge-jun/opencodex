 ...
 *  - This test REFRESHES but does NOT persist the rotated token back to the
 *    store and does NOT call logout, so it cannot destroy the real session.
...
describe.skipIf(!LIVE)("Nous Portal live verification (opt-in, no key shared)", () => {
  test("real-account refresh returns a rotated token and live catalog is reachable", async () => {
