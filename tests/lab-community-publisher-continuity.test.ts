...
describe("CL-10 publisher continuity", () => {
  test("same content from two publishers coexists and revokes independently", () => {
    ...
    const signerA = getOrCreatePublicPublisher(publisherA);
    const signerB = getOrCreatePublicPublisher(publisherB);
    const unsigned = unsignedBundle();
    const bundleA = signPublicEvidenceBundle(unsigned, signerA);
    const bundleB = signPublicEvidenceBundle(unsigned, signerB);
