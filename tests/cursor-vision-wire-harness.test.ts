  ...
  for (const turnId of run?.conversationState?.turns ?? []) {
    ...
    for (const stepId of turn.turn.value.steps) {
      ...
      for (const item of result.value.content) {
        if (item.content.case === "image") return item.content.value.data;
      }
    }
  }
  return undefined;
...
describe("Cursor vision wire harness", () => {
  test("grok attach + view_image paths keep non-empty PNG bytes on the wire", () => {
    ...
    const viewWireBytes = viewImageToolResultBytes(viewBytes);
    expect(viewWireBytes).toBeDefined();
    expect(viewWireBytes!.byteLength).toBeGreaterThan(0);
    expect(Array.from(viewWireBytes!)).toEqual(Array.from(imageBytes));
  });
});
