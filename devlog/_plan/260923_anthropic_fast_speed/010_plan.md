...
- D5 Refusal downgrade (final, after reflection 030 and audit 040). Recognition is narrow: status 400 or 429 whose Anthropic error message names fast mode or the `speed` parameter (probe strings: `Usage credits are required for fast mode.`, `Fast mode is not enabled for your organization`, `does not support the \`speed\` parameter`), or a 429 carrying an `anthropic-fast-*-remaining: 0` header. Generic 429/529 keep today's path. The body is read from a bounded clone, so the original refusal survives if the resend is not admitted.
  ...
  - The resend is a visible, paced, attempt-logged send with its recovery kind and a real shared-budget charge; a test pins that a spent budget refuses it and returns the original refusal.
