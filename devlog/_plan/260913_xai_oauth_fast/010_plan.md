...
- D4 Echo relay: today the upstream service_tier echo reaches attempt telemetry but not the client on chat-wire paths (probe: 4.3/4.20/build/composer client bodies lack the field; 4.6/4.5 responses-wire bodies carry it). Add the echo to (a) the responses-lane assembly for chat upstreams (src/bridge.ts buildResponseJSON / bridgeToResponsesSSE, fed by the openai-chat adapter result) and (b) the chat-inbound relay (src/server/chat-completions.ts). Sizing rule: if the plumbing crosses more than the adapter-event → bridge → endpoint seams, split relay to a follow-up PR in the same unit and ship D1-D3 here, recording the gap.
...
2. Relay (D4): src/adapters/openai-chat.ts (surface upstream service_tier beyond telemetry), src/bridge.ts (buildResponseJSON:1580/1596 + bridgeToResponsesSSE:207 carry it), src/server/chat-completions.ts chat-projection functions collectChatCompletion:473 and responsesJsonToChatCompletion:497 (chat-inbound relay). Audit note: no adapter event/result carries service_tier today (openai-chat.ts:1778-1781, 2080-2082 observe it into attempt telemetry only; AdapterTierMetadata is telemetry per src/adapters/base.ts:118) — D4 sits exactly at the split-rule boundary; apply the rule honestly.
...
- C6 relay: a chat-upstream response carrying service_tier surfaces it (a) in the responses-lane client body and (b) in the chat-inbound client body (bridge/endpoint tests; skipped with recorded reason only if D4 splits).
