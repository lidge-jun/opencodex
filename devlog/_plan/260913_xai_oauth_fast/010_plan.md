...
2. Relay (D4): src/adapters/openai-chat.ts (surface upstream service_tier on the adapter result/event), src/bridge.ts (buildResponseJSON + SSE carry it), src/server/chat-completions.ts (chat-inbound relay).
...
- C5 enrichProviderFromRegistry backfills chatServiceTier/modelSupportsServiceTier into a saved xai config missing them; explicit config values win (capability test).
