...
| `src/config/admitted-identity.ts` | Which configuration a derived artifact was built from: the admission snapshot's file digest paired with a canonical structural digest of the in-memory configuration object. Neither term alone is enough — the file cannot see which object a caller is holding, and the object cannot say which file it came from. Null is unprovable and every caller fails closed on it. |
