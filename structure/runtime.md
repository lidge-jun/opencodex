...
uninstall still restore. `src/service/cli.ts` removes the service token on uninstall only when persisted client state is disconnected. Connected, invalid, or mismatched client state retains an existing token; an absent token is reported as absent, and lock, state-read, or deletion failure leaves cleanup unverified. Client lifecycle and config mutation locks exclude a concurrent connect or disconnect during this decision.
