...
| `src/codex/shim.ts` | `codex-shim install` exits nonzero when the resulting shim is unhealthy or installation is refused, so the dashboard propagates the failure reason; an already-installed healthy shim succeeds. Codex autostart shim: replaces the `codex` binary with a wrapper that auto-starts the proxy on demand. It skips startup for management subcommands even when value-taking global flags precede the subcommand, and transactionally restores complete, stable external launcher replacements without a watcher or PATH rediscovery. |
