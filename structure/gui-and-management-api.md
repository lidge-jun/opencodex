...
`GET /healthz`, `/readyz`, the packaged GUI/SPA routes, and `/api/machine/*`. A link-transport client
also relays the link data routes to its Home and serves its own read-only `GET /api/link/status`
