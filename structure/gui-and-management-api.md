...
| Models and visibility | `src/server/management/model-routes.ts` — `GET /api/models`, `PUT /api/disabled-models`, `PUT /api/model-visibility`, `GET/PUT /api/global-model-visibility`, `PUT /api/selected-models`, `GET/POST /api/custom-models`. The Models page controls exact model IDs globally; Providers → Models controls only one provider's entry. Effective row visibility is the union of both hides. Visibility writes trigger catalog sync through the owning server path. |
