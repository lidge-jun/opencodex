...
| Catalog | `disabledModels`, `customModels`, `modelCacheTtlMs`, `providerContextCaps`, `contextCapValue`, `codexAccountPickerEnabled` | Catalog state is derived; config only records intent. The account-picker flag controls discovery only; exact selector bindings remain routable. |
