...
| Provider compatibility / Zen keyless identity | `src/adapters/provider-compatibility.ts`, `src/adapters/opencode-free-compatibility.ts`, `src/adapters/opencode-free-session.ts`, `src/adapters/opencode-free-tools.ts` | The generic request-transform contract runs after wire serialization; provider implementations own their quirks. Zen's non-forward transform supplies a thread- or request-lane-scoped identity, anonymous auth only when Authorization is absent, User-Agent repair, and gate declarations; Muse Spark contributor-free ids ride the Responses wire via the registry wire default. |
