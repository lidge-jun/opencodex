...
| `src/providers/resolved-model-policy.ts` | Pure static provider/model policy resolution. It detaches and freezes registry defaults, operator overrides, hard wire pins, aliases, and explicit false/empty values with field-level provenance. Callers supply the admitted registry row and transport-match decision; the module imports no router, catalog, server, credential, account, quota, health, or request state. Observed model limits remain outside the result and use a separate call-local clamp projection. |
