...
| `src/adapters/command-code.ts`, `src/adapters/command-code-tool-text.ts`, `src/adapters/command-code-restored-schema.ts` | Command Code OAuth NDJSON translation. MiMo 2.6 text, native calls, reasoning, and terminal decisions share one bounded output queue. Markup is deduplicated against matching native calls; text-only restoration requires a clean finish, a declared tool, and arguments validated against supported schema constraints. Unsupported constraints fail closed. Failed, filtered, truncated, and unterminated turns release held markup as text. |
...
excluded from later refreshes of that model. Configuration can take precedence, but only when the provider declares
