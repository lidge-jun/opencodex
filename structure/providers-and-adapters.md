...
`src/adapters/opencode-free-session.ts` mints the keyless Zen tier identity for the canonical `https://opencode.ai/zen/v1` endpoint from the shared Chat transport (translated adapter and native fast path alike) and the Responses passthrough (`src/adapters/openai-responses/passthrough.ts`): a stable-per-Codex-thread `x-opencode-session` in the OpenCode session shape (`ses_` + 12 hex + 14 base62), the anonymous `Bearer public`, and a repair of exactly the bare default User-Agent to a verified versioned one — applied only when the operator has not set those headers and only when no API key is configured. `src/adapters/opencode-free-tools.ts` appends the missing never-invoked `shell`/`read` gate declarations the tier requires, keyless only, and builds the guidance text for mistaken calls (naming the turn's real alternatives, never upstream internals): the translated lane substitutes it in a registry-composed Chat wrapper, the native Responses lane in the undeclared-tool guard rewrite, both firing only for gate names the client never declared. The `opencode-free` registry entry routes the Responses-only Muse Spark contributor-free ids to `openai-responses` through `modelWireDefaults` on every inbound; every other Zen id stays on Chat.
