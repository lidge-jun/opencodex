...
| --- | --- | --- |
| `.github/workflows/ci.yml` | Any `pull_request`; runtime/package `push` to `main`/`preview`/`dev`; manual dispatch | Linux runs four suite shards plus `gates`; macOS runs two shards. Manual `lane=release-gates` keeps ordinary job conditions, including three-platform keyring and conditional packaging smoke, but skips the Windows suite and unsharded macOS control. Default `all` (or empty) still runs both diagnostics; `macos-control` runs the control without Windows suite shards. Aggregate `ci` accepts intentional diagnostic skips and does not prove Windows suite success. `npm-global-smoke` stays GitHub-hosted because it mutates the global package prefix. Release eligibility still requires successful push-event CI on the exact release SHA; a manual lane does not authorize publishing. |
