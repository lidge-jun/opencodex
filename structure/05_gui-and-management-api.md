...
listener serves packaged GUI/SPA routes, `GET`/`POST /opencodex-session`, `/api/*`, and the single
bearer-authenticated `/remote-workspace/agent` WebSocket opened outbound by a paired Executor; all
data, health, readiness, other WebSocket, and unknown-static routes receive a JSON 404 before dispatch.
...
| Remote Workspace | `src/server/management/remote-workspace-routes.ts` owns Hub-only device, runtime, and model-session views. Pairing grants, session start/prompt/stop, and revoke are GUI-session consent operations; the machine pairing exchange and outbound agent WebSocket carry only device-scoped enrollment/transport authority. Session execution stays bound to one device, root, access mode, and capability-filtered tool set. |
