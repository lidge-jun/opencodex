...
| Providers | Create/update/delete ordinary provider configs and enrich registry metadata. The reserved `openai` card exposes Pool(default)/Direct account mode; `openai-apikey` remains the separate API route. Canonical OpenRouter cards can read sanitized per-model endpoint tags through `GET /api/openrouter/model-providers` using the configured API key and persist exact-model routing with the existing provider PATCH. Discovery permits eight unique active flights and returns a bounded busy response above that cap while identical callers share one flight. |
