...
is not one of our secrets never proves proxy admission. When a dedicated header independently
admits the request, that caller-owned bearer can remain Codex Direct passthrough or serve as
request-scoped `__main__` Pool auth; it is never persisted. The two bearer domains never mix.
