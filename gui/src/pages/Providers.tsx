...
import ProviderSummaryPanel from "../components/provider-catalog/ProviderSummaryPanel";
...
import { oauthAccountDisplayLabel, partitionAccountProviderRows } from "../provider-workspace/auth";
...
export default function Providers({ apiBase }: { apiBase: string }) {
  ...
  const { connected: connectedAccountRows, available: visibleAccountRows } = partitionAccountProviderRows(
    addModalAccountRows,
    accountLoginStatus,
      ...
      {!editing && (
        <div className="prov-auth-grid">
          <ProviderSummaryPanel
            ...
            title={t("prov.oauthProviders")}
            description={t("prov.accountLoginHint")}
            emptyMessage={connectedAccountRows.length === 0 ? t("prov.noConnectedAccounts") : undefined}
            addLabel={t("prov.connectAccountTitle")}
            addDescription={t("prov.connectAccountDesc")}
            onAdd={() => {
              setAccountPickerQuery("");
          ...
          <ProviderSummaryPanel
            ...
            title={t("prov.apiProviders")}
            description={t("prov.apiProvidersHint")}
            emptyMessage={keyCardProviders.length === 0 ? t("prov.noApiProviders") : undefined}
            addLabel={t("prov.addApiProvider")}
            addDescription={t("prov.addApiProviderDesc")}
            onAdd={() => {
