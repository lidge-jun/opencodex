...
"common.discard": "버리기",
  ...
  "compactionRouting.providerWarningScoped": "이 설정을 사용하면 {sources}의 압축 요청만 전체 대화 내용을 요약을 위해 {provider}로 전송하며, 다른 대화는 계속 자체 모델로 압축합니다.",
  "compactionRouting.comboWarningScoped": "이 설정을 사용하면 {sources}의 압축 요청만 전체 대화 내용을 요약을 위해 콤보 {combo}로 전송하며, 다른 대화는 계속 자체 모델로 압축합니다. 콤보는 대상({providers})을 순서대로 시도해 먼저 응답한 하나를 사용하므로, 그중 어느 것이든 이 대화를 받을 수 있습니다.",
