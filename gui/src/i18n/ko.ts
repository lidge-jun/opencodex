...
export const ko: Record<TKey, string> = {
  ...
  "codexAuth.namespacesTitle": "각 Codex 계정을 모델 선택기에 별도로 표시",
  "codexAuth.namespacesOnDesc": "사용 가능한 Codex 계정마다 각 GPT 모델이 별도 항목으로 표시됩니다. Main은 기본 제공 로그인을 뜻하며 공개 선택기 라벨은 저장된 계정 하나에 비공개로 연결됩니다. 추가 계정은 사용자가 정한 표시 이름을 사용하고, 표시 이름이 없으면 안정적인 개인정보 보호 라벨 pXXXXXX를 사용합니다. 저장된 계정 ID와 이메일은 사용하지 않습니다. 기존 라벨과 사용자 지정 라벨은 바뀌지 않습니다. 선택한 대화는 해당 계정에 고정되고 OpenCodex는 다른 계정으로 전환하거나 대체하지 않습니다.",
  "codexAuth.namespacesOffDesc": "여러 Codex 계정 중 대화에 사용할 계정을 로그아웃 없이 명시적으로 선택하려면 켜세요. 꺼도 이 항목만 숨겨지며 계정은 삭제되지 않습니다.",
  "codexAuth.namespacesCompatibilityNote": "모델 선택기의 일반 GPT 항목을 명시적인 계정별 항목으로 바꿉니다. 기존 대화, 사용자 지정 계정 라벨, 저장된 모델 선택은 현재 라우팅을 유지하고, 접두사 없는 GPT 모델 ID는 기존 풀 또는 직접 동작을 유지합니다.",
  "codexAuth.namespacesSaved": "계정별 모델 선택 항목이 업데이트되었습니다. 목록이 오래된 경우 Codex를 다시 여세요.",
  "codexAuth.namespacesSaveFailed": "모델 선택기를 업데이트하지 못했습니다. 이전 설정은 유지됩니다.",
  "codexAuth.add": "추가",
  ...
  "codexAuth.catalogRefreshPending": "계정 변경은 저장되었지만 모델 선택기를 새로 고치지 못했습니다. `ocx sync`를 실행해 다시 시도하세요.",
