...
export const ja: Record<TKey, string> = {
  ...
  "navGroup.resources": "リソース",
  "navGroup.observability": "可観測性",
  "navGroup.automation": "自動化",
  "navGroup.system": "システム",
  // sidebar / nav / common
  ...
  "common.retry": "再試行",
  "auth.adminTokenTitle": "hubapi 管理者トークン (OPENCODEX_ADMIN_AUTH_TOKEN)",
  ...
  // startup health
  "startup.title": "起動安全性",
  "startup.subtitle": "再起動後にローカルプロキシへの接続が再接続ループになる前に、Codex が hubapi へ到達できるか確認します。",
  "startup.refresh": "更新",
  ...
  "startup.summary.protected": "再起動後も hubapi を利用できます",
  ...
  "startup.riskDetailCustomLocal": "Codex はカスタムローカルゲートウェイを参照しています。hubapi はその再起動ライフサイクルを管理・検証できません。",
  ...
  "startup.codexRuntime.clampHidden": "hubapi が Codex {version} を使用したため、一部の reasoning effort オプションが非表示になりました。",
  "startup.codexRuntime.clampHiddenWithEfforts": "hubapi が Codex {version} を使用したため、一部の reasoning effort オプションが非表示になりました（削除: {efforts}）。",
  "startup.codexRuntime.olderBinary": "hubapi は古い Codex バイナリ（{version}）を使用しています。より新しいインストールが利用可能です。",

  // dashboard
  "dash.subtitle": "ローカル hubapi プロキシ、そのプロバイダー、Codex にルーティングされるモデルのライブ状態です。",
  ...
  "dash.routes.title": "最近のルート証拠",
  "dash.routes.hint": "記録済みリクエストのみ。構成プレビューとは分離されています。",
  "dash.routes.viewAll": "ログを表示",
  "dash.routes.unavailable": "永続リクエスト履歴を利用できません。",
  "dash.routes.empty": "永続化されたリクエストはまだありません。",
  "dash.routes.unknown": "ルートトレースなし",
  "dash.routes.attempts": "{count} 回の試行",
  ...
  "dash.codexAutoStart": "Codex と一緒に hubapi を起動",
  ...
  "dash.shadowCallTooltip": "Codex App はスレッドタイトル生成、コミットメッセージ生成、スキルオーケストレーションをバックグラウンドで呼び出します。使われるモデルはクライアントのバージョンによって変わるため、hubapi は {models} をまとめて傍受します。これをオンにすると、それらの呼び出しを選択したモデルにリダイレクトします。",
  ...
  "dash.maintenanceHint": "Codex のモデルカタログを更新するか、より新しい hubapi リリースをインストールします。",
  ...
  "dash.projectConfigTitle": "プロジェクトの Codex 設定が hubapi をバイパスします",
  "dash.projectConfigHint": "これらのリポジローカル設定は hubapi プロキシを上書きします(例: OpenCode Go に直接ルーティング)。~/.codex/config.toml のルーティングがそのプロジェクトで適用されるように削除してください。",
  "dash.checkUpdate": "更新を確認",
  "dash.updateTitle": "hubapi を更新",
  ...
  "dash.updateStatus.running": "hubapi を更新しています。",
  ...
  // providers
  "prov.subtitle": "hubapi が Codex にルーティングする上流プロバイダーを設定します。アカウントでログインするか、プロバイダーを追加、または生の設定を編集します。",
  "prov.add": "プロバイダーを追加",
  ...
  "oauthTos.anthropicBody": "hubapi のような第三者プロキシ経由で Claude サブスクリプションの OAuth トークンを直接再利用することは、Anthropic がサポートする統合ではなく、アクセス制限につながる可能性があります。Claude サブスクリプションを使用するサポートされた Agent SDK 統合は別物です。",
  "oauthTos.highBody": "hubapi は第三者 OAuth パス経由で {provider} に接続します。サポート外の利用はアクセス制限や停止につながる可能性があります。",
  "oauthTos.elevatedBody": "hubapi は非公式 OAuth パス経由で {provider} に接続します。可能な場合は公式クライアントを使用してください。異常または自動化されたトラフィックは悪用とみなされ、アクセスが制限または停止される可能性があります。",
  "oauthTos.saferPath": "より安全な選択肢: 代わりに hubapi で API キーを設定してください。",
  ...
  "models.subtitle": "Codex に表示するモデルを切り替えます — ネイティブ GPT パススルーとルーティングプロバイダー、プロバイダー別(ヘッダーをクリックで折りたたみ)。非表示モデルはカタログとピッカーから外れますが、正確な id での直接呼び出しは可能です。変更は次回の Codex ターンで適用 — hubapi は Codex の 5 分間モデルキャッシュを無効化するので再起動は不要です。",
  ...
  // subagents
  "sub.subtitle": "Codex の {cmd} は最初の 5 モデル(優先度順)のみをオーバーライドとして通知します。ここで最大 5 つを選んでください — ネイティブ gpt またはルーティング — hubapi がカタログ優先度を設定し、これらが先頭に来るようにします。他のモデルも正確な名前で呼び出し可能です; これは表示のみを制御します。",
  "sub.featured": "おすすめ",
  ...
  "logs.tabLogs": "ログ",
  "logs.tabDebug": "デバッグ",
  "logs.subtitle": "ローカル hubapi プロキシを経由した永続化リクエスト（新しい順）。",
  "logs.history.indexed": "永続化された {count} 件のリクエストを索引化",
  "logs.history.repaired": "履歴インデックスを修復しました",
  "logs.history.loadOlder": "古いリクエストを読み込む",
  "logs.history.loadError": "古いリクエストを読み込めませんでした。",
  ...
  "logs.filter.advanced": "永続履歴フィルター",
  "logs.filter.provider": "プロバイダー ID",
  "logs.filter.model": "最終モデル",
  "logs.filter.requestedModel": "要求モデル",
  "logs.filter.status": "HTTP ステータス",
  "logs.filter.protocol": "受信プロトコル",
  "logs.filter.apiKey": "API キー ID",
  "logs.filter.profile": "ルーティングプロファイル ID",
  "logs.filter.fallback": "フォールバック使用",
  "logs.filter.fallback.any": "すべて",
  "logs.filter.fallback.yes": "はい",
  "logs.filter.fallback.no": "いいえ",
  "logs.filter.apply": "フィルターを適用",
  "logs.filter.clearAll": "すべて消去",
  ...
  "logs.detail.route.loading": "記録済みのルート証拠を読み込み中…",
  "logs.detail.route.loadError": "ルート証拠を読み込めませんでした。最終プロバイダーから経路を推測しません。",
  "logs.detail.route.requested": "要求モデル",
  "logs.detail.route.final": "最終ターゲット",
  "logs.detail.route.outcome": "結果",
  ...
  "integrations.subtitle": "クライアントを hubapi に接続し、認証情報の管理とクライアント設定の復元を行います。",
  ...
  "integrations.codex.body": "Codex の接続はプロキシサービスが管理します。hubapi を起動すると適用され、サービスを停止するとネイティブのルーティングに戻ります。",
  ...
  "integrations.dialog.desktop.changes": "{path} に hubapi 管理のゲートウェイプロファイルがある場合、Desktop は先に認証情報のない標準プロファイルを選択し、その後で古いプロファイルとバックアップを削除します。",
  "integrations.dialog.desktop.breakage": "Claude Desktop は hubapi 経由のモデルではなく、標準の Claude に戻ります。",
  "integrations.dialog.desktop.undo": "再度有効にすると、保存済みのモデル割り当てから hubapi プロファイルを再生成します。",
  ...
  "integrations.native.error.desktopCleanupIncomplete": "Claude Desktop は標準モードを指していますが、古い hubapi 認証情報ファイルが残っています: {paths}。",
  ...
  "integrations.detail.grokAbsent": "設定に hubapi ブロックがありません",
  "integrations.dialog.grok.title": "Grok Build 連携を解除しますか？",
  "integrations.dialog.grok.changes": "{path} から、hubapi が印を付けたブロックだけを削除します。ブロック外に直接書いた内容はそのまま残します。",
  "integrations.dialog.grok.breakage": "解除すると、Grok Build から hubapi のモデルエイリアスが消えます。xAI アカウントで使用していたモデルはそのままです。",
  "integrations.dialog.grok.undo": "hubapi が loopback アドレスで実行中なら、再び有効にしたとき、現在利用できるモデル一覧からブロックを新しく書き込みます。",
  "integrations.dialog.grok.confirm": "解除",
  "integrations.native.msg.nonLoopbackRemoved": "Grok Build を自動登録できるのは、hubapi が loopback アドレスで実行中の場合だけです。loopback アドレスを指していた以前のブロックを削除しました。",
  "integrations.native.msg.nonLoopbackRemovedNoop": "Grok Build を自動登録できるのは、hubapi が loopback アドレスで実行中の場合だけです。削除する以前のブロックはありませんでした。",
  "integrations.native.msg.nonLoopbackSuperseded": "Grok Build を自動登録できるのは、hubapi が loopback アドレスで実行中の場合だけです。その間に別の場所から設定へ新しいブロックが書かれたため、現在ファイルにあるブロックはこのリクエストが作成したものではありません。",
  "integrations.native.error.orphanedMarker": "{path} に hubapi の開始マーカーはありますが、終了マーカーがありません。ブロックの終端を特定できないため、ファイルは変更していません。",
  ...
  "integrations.onboarding": "適用時は、先にバックアップを保存してから hubapi プロバイダーブロックを 1 つだけ書き込みます。無効化時はそのブロックだけを削除し、保存されたスナップショットから復元できます。",
  "integrations.empty.title": "インストール済みのクライアントが検出されませんでした",
  "integrations.empty.body": "対応クライアントをインストールしてから、ここに戻って hubapi を適用してください。",
  ...
  "integrations.error.conflict": "hubapi の書き込み後に設定が変更されました。何も削除していません。",
  ...
  "integrations.bulk.body": "hubapi が所有するブロックだけを削除します。各クライアントについて、変更前のスナップショットを保存します。",
  ...
  "integrations.semantics.dsh": "hubapi が管理するのは $DSH_HOME/settings.yaml 内の llm-pi-ai.providers.opencodex だけです。DSH はこのプロバイダーをホットリロードし、既定のモデルと deepseek-official は変更しません。現在はループバック専用で、実際の認証情報は書き込みません。",
  ...
  "accountPool.strategyDesc": "hubapi が新規/未紐付けタスクへアカウントを割り当てる方法です。",
  ...
  // api access page
  "api.title": "プロキシ API",
  "api.subtitle": "生成した API キーで外部アプリから hubapi プロキシに接続します。認証は {authHeader} ヘッダーで行い、エンドポイントごとに受け付けるヘッダーは下の表のとおりです。",
  "api.proxy.title": "統合プロキシゲートウェイ",
  "api.proxy.online": "オンライン — API リクエストを受付中",
  "api.proxy.flowLabel": "プロキシのリクエストフロー",
  "api.proxy.clients": "クライアント",
  "api.proxy.providers": "プロバイダー",
  "api.proxy.baseUrl": "Base URL",
  "api.proxy.protocols": "受信プロトコル",
  "api.proxy.auth": "認証",
  ...
  "claude.smallFastModelNativeWarning": "未設定の場合、hubapi はヘルパーモデルの上書きを設定しません。Claude Code がネイティブの Sonnet モデルを使用し、ネイティブプロバイダーで料金が発生する可能性があります。",
  ...
  "grok.subtitle": "hubapi が Grok 設定に登録したモデルです。",
  "grok.loading": "Grok の状態を読み込み中…",
  "grok.loadFail": "Grok 設定を読み取れませんでした。",
  "grok.notConfiguredTitle": "Grok Build が未設定です",
  "grok.notConfiguredHint": "Grok をインストールしてプロキシを再起動すると、hubapi が管理ブロックを次の場所に書き込みます:",
  ...
  "cws.howTitle": "仕組み",
  "cws.howBody": "Codex に combo/<id> を要求します。hubapi はターゲットを選び、再試行可能な上流の失敗時のみホップします。利用可能なターゲットが残っていない場合、グローバルなデフォルトプロバイダーを使わずにフェイルクローズします。",
