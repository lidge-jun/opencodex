...
"codexSet.dialog.notRendered": "確認したターンでは、このレイヤーは何も送信していません。各セクションは内容が変わったときだけ再送されるため、一度の確認では欠けて見えることがあります。",
  ...
  "sub.taskRecovery": "暗号化された V2 タスクを復元",
  "sub.taskRecoveryHint": "ネイティブ ChatGPT 親からルーティングモデルへ委任する場合に使います。追加の認証済み ChatGPT リクエストにより、クォータ消費と遅延が増える可能性があります。復元テキストは制限付きメモリキャッシュにのみ保持され、失敗時は安全に停止します。",
  "sub.taskRecoveryConfirm": "実験的な暗号化タスク復元を有効にしますか？追加の ChatGPT クォータと遅延が発生し、非公開のバックエンド動作に依存します。",
  ...
  "pws.openrouter.title": "モデル別プロバイダールーティング",
  "pws.openrouter.hint": "モデルごとに使用する OpenRouter エンドポイントを選択します。検出には認可済み OpenRouter Management Key が必要ですが、実際のルーティングは設定済み API キーを使います。",
  "pws.openrouter.load": "プロバイダーを読み込む",
  "pws.openrouter.mode": "ルーティング動作",
  "pws.openrouter.inherit": "プロバイダー既定値を継承",
  "pws.openrouter.order": "選択したプロバイダーを優先",
  "pws.openrouter.only": "選択したプロバイダーのみ使用",
  "pws.openrouter.allowFallbacks": "選択後に OpenRouter フォールバックを許可",
  "pws.openrouter.managementKeyRequired": "エンドポイント検出には認可済み OpenRouter Management Key が必要です。既存の手動ルーティングは変更されません。",
  "pws.openrouter.keyRequired": "プロバイダーを検出する前に OpenRouter API キーを追加してください。",
  "pws.openrouter.loadFailed": "このモデルのプロバイダーを読み込めませんでした。",
  "pws.openrouter.loaded": "{count} 件のプロバイダーエンドポイントを読み込みました。",
  "pws.openrouter.notReturned": "最新の検出結果にありません",
  "pws.openrouter.saved": "モデルのプロバイダールーティングを保存しました。",
  "pws.openrouter.cache": "キャッシュ",
};
