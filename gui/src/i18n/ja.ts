...
export const ja: Record<TKey, string> = {
  ...
  "codexAuth.resetCliOnly": "リセットクレジットを使用できるのはループバックのダッシュボードだけです。リモートダッシュボードでは OpenCodex ホスト上でローカル CLI の同意フローを使用してください。",
  "codexAuth.confirmResetTitle": "リセットクレジットを使用しますか?",
  "codexAuth.confirmResetDesc": "現在のレート制限を即座にリセットします。確認後、所有者の同意を証明するため OpenCodex 管理者トークンを再入力します。残り {count} クレジットです。",
  ...
  "codexAuth.resetHistoryFull": "リセットクレジット操作履歴が上限に達しました。再試行する前に、メンテナーに容量の拡張または承認済みの履歴削除ポリシーの適用を依頼してください。",
