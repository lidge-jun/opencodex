...
"codexSet.dialog.notRendered": "確認したターンでは、このレイヤーは何も送信していません。各セクションは内容が変わったときだけ再送されるため、一度の確認では欠けて見えることがあります。",
  ...
  "remoteLink.findHome.notice": "接続すると、このコンピューターの OpenCodex が再起動します。実行中の Codex ターンは先に完了し、再起動中の最大 1 分間は新しいリクエストが失敗することがあります。その後も Codex は同じローカルアドレスを使い続け、Home が自身のプロバイダーとアカウントで応答します。",
  ...
  "remoteLink.restart.slow": "再起動に通常より時間がかかっています。このページは確認を続け、子が応答すると自動的に再読み込みされます。OpenCodex が停止している場合は、もう一度起動してください。",
  ...
  "remoteLink.error.fingerprint_failed": "SSH ホストキーを読み取れませんでした。接続テストをもう一度実行してください。",
  "remoteLink.error.forbidden": "このダッシュボードセッションではリモートリンクを管理できません。このコンピューター上でダッシュボードを開く（スタンドアロン環境）か、ペアリング済みのハブセッションを使用してください。",
  ...
  "remoteLink.error.link_unavailable": "このランタイムではリモートリンクを利用できません。OpenCodex を再起動してから再試行してください。",
  "remoteLink.error.listener_unavailable": "このコンピューターでリモートリンクのリスナーを起動できませんでした。ポートの競合がないか確認してから再試行してください。",
  "remoteLink.error.probe_failed": "SSH ホストに接続できませんでした。SSH キーで認証できるか、必要な ProxyCommand ヘルパーがインストールされているかを確認してください。",
  "remoteLink.error.remote_connect_failed": "リモートのコンピューターがこの Home に接続できませんでした。そのコンピューターの OpenCodex サービスを確認してから再試行してください。",
  "remoteLink.error.remote_disconnect_failed": "リモートリンクのリクエストを完了できませんでした。",
  "remoteLink.error.remote_ocx_missing": "リモートのコンピューターで ocx が見つかりませんでした。そこに OpenCodex をインストールするか、非対話 SSH セッションの PATH に ocx があることを確認してください。",
  "remoteLink.error.remote_ocx_outdated": "リモートのコンピューターの OpenCodex が古く、リモートリンクに対応していません。そのコンピューターで ocx update を実行して 2.66.0 以降にしてから再試行してください。",
  "remoteLink.error.remote_ocx_unrecognized": "リモートのコンピューターが OpenCodex のバージョンを返しませんでした。リモートリンクには macOS または Linux 上の OpenCodex 2.66.0 以降が必要です。",
  "remoteLink.error.remote_port_failed": "リモートのコンピューターがリンクポートを返しませんでした。そのコンピューターで OpenCodex 2.66.0 以降が動作しているか確認してから再試行してください。",
  "remoteLink.error.tailscale_session_refused": "リモートリンクのリクエストを完了できませんでした。",
  "remoteLink.error.version_probe_failed": "SSH 経由でリモートのコンピューターの ocx を実行できませんでした。SSH アクセスを確認してから再試行してください。",
