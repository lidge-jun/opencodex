...
export const ja: Record<TKey, string> = {
  ...
  "codexAuth.namespacesOnDesc": "利用可能な Codex アカウントごとに各 GPT モデルが個別に表示されます。追加アカウントのラベルはカードに表示されたローカル ID から作られ、組み込みログインは Main から始まります。ラベルが重なる場合は数字が付きます。選択した会話はそのアカウントに固定され、OpenCodex が別のアカウントへ切り替えたりフォールバックしたりすることはありません。",
  "codexAuth.namespacesOffDesc": "複数の Codex アカウントから、ログアウトせずに会話で使うアカウントを明示的に選びたい場合に有効にします。無効にしてもこれらの項目が非表示になるだけで、アカウントは削除されません。",
  "codexAuth.namespacesCompatibilityNote": "モデル選択の通常の GPT 項目を、アカウント別の明示的な項目に置き換えます。既存の会話と保存済みのモデル選択は現在のルーティングを維持し、プレフィックスなしの GPT モデル ID は従来のプールまたは直接動作を維持します。",
