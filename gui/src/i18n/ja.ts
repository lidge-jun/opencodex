...
"codexSet.dialog.notRendered": "確認したターンでは、このレイヤーは何も送信していません。各セクションは内容が変わったときだけ再送されるため、一度の確認では欠けて見えることがあります。",
  ...
  "pws.openrouter.hint": "モデルごとに使用する OpenRouter エンドポイントを選択します。検出とルーティングには設定済みの OpenRouter API キーを使います。",
  ...
  "pws.openrouter.authorizationFailed": "OpenRouter がエンドポイント検出用の設定済み API キーを拒否しました。既存の手動ルーティングは変更されません。",
