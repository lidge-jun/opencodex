...
export const ja: Record<TKey, string> = {
  ...
  "codexAuth.namespacesTitle": "各 Codex アカウントをモデル選択に個別表示",
  "codexAuth.namespacesOnDesc": "利用可能な Codex アカウントごとに各 GPT モデルが個別に表示されます。Main は組み込みログインを表し、公開される各ピッカーラベルは保存済みアカウント 1 件に非公開で対応します。追加アカウントにはユーザーが指定した表示名を使い、表示名がなければ安定したプライバシー保護ラベル pXXXXXX を使います。保存済みアカウント ID やメールアドレスは使用しません。既存またはカスタムのラベルは変更されません。選択した会話はそのアカウントに固定され、OpenCodex が別のアカウントへ切り替えたりフォールバックしたりすることはありません。",
  "codexAuth.namespacesOffDesc": "複数の Codex アカウントから、ログアウトせずに会話で使うアカウントを明示的に選びたい場合に有効にします。無効にしてもこれらの項目が非表示になるだけで、アカウントは削除されません。",
  "codexAuth.namespacesCompatibilityNote": "モデル選択の通常の GPT 項目を、アカウント別の明示的な項目に置き換えます。既存の会話、カスタムのアカウントラベル、保存済みのモデル選択は現在のルーティングを維持し、プレフィックスなしの GPT モデル ID は従来のプールまたは直接動作を維持します。",
  "codexAuth.namespacesSaved": "アカウント別のモデル選択項目を更新しました。表示が古い場合は Codex を開き直してください。",
  "codexAuth.namespacesSaveFailed": "モデル選択を更新できませんでした。以前の設定は維持されます。",
  "codexAuth.add": "追加",
  ...
  "codexAuth.catalogRefreshPending": "アカウントの変更は保存されましたが、モデルピッカーを更新できませんでした。`ocx sync` を実行して再試行してください。",
