...
export const ru: Record<TKey, string> = {
  ...
  "codexAuth.namespacesOnDesc": "Для каждого доступного аккаунта Codex каждая GPT-модель показывается отдельно. Метка добавленного аккаунта создаётся из локального ID на его карточке, а метка встроенного входа начинается с Main. При совпадении добавляется числовой суффикс. Выбранный диалог закрепляется за этим аккаунтом; OpenCodex не переключается и не использует другой аккаунт как резервный.",
  ...
  "codexAuth.namespacesCompatibilityNote": "Обычные строки GPT в списке моделей заменяются явными вариантами для каждого аккаунта. Существующие диалоги и сохранённый выбор модели сохраняют текущее направление, а ID GPT без префикса — режим Пул или Прямой.",
