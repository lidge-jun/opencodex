...
export const ru: Record<TKey, string> = {
  ...
  "api.tunnelModeQuick": "Quick Tunnel · Временный доступ для разработки: до 200 одновременно выполняемых запросов, без поддержки SSE. Публичный URL меняется после перезапуска.",
  "api.tunnelModeNamed": "Named Tunnel · Стабильный публичный доступ с поддержкой SSE, подходящий для потоковых запросов.",
