...
export const ru: Record<TKey, string> = {
  ...
  "codexAuth.namespacesTitle": "Показывать каждый аккаунт Codex отдельно в списке моделей",
  "codexAuth.namespacesOnDesc": "Для каждого доступного аккаунта Codex каждая GPT-модель показывается отдельно. Main обозначает встроенный вход, а каждая публичная метка в селекторе приватно сопоставляется одному сохранённому аккаунту. Для добавленного аккаунта используется заданное пользователем отображаемое имя, а без него — стабильная безопасная метка вида pXXXXXX. Сохранённые ID аккаунтов и адреса электронной почты не используются. Существующие и пользовательские метки не изменяются. Выбранный диалог закрепляется за этим аккаунтом; OpenCodex не переключается и не использует другой аккаунт как резервный.",
  "codexAuth.namespacesOffDesc": "Включите, если у вас несколько аккаунтов Codex и вы хотите явно выбрать аккаунт для диалога без выхода из системы. При отключении эти строки только скрываются; аккаунты не удаляются.",
  "codexAuth.namespacesCompatibilityNote": "Обычные строки GPT в списке моделей заменяются явными вариантами для каждого аккаунта. Существующие диалоги, пользовательские метки аккаунтов и сохранённый выбор модели сохраняют текущее направление, а ID GPT без префикса — режим Пул или Прямой.",
  "codexAuth.namespacesSaved": "Отображение аккаунтов в списке моделей обновлено. Если список устарел, откройте Codex заново.",
  "codexAuth.namespacesSaveFailed": "Не удалось обновить список моделей. Предыдущая настройка сохранена.",
  ...
  "codexAuth.catalogRefreshPending": "Изменение аккаунта сохранено, но список моделей не удалось обновить. Выполните `ocx sync`, чтобы повторить попытку.",
