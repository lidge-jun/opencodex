...
export const ru: Record<TKey, string> = {
  ...
  "navGroup.resources": "Ресурсы",
  "navGroup.observability": "Наблюдение",
  "navGroup.automation": "Автоматизация",
  ...
  "auth.adminTokenTitle": "Токен администратора hubapi (OPENCODEX_ADMIN_AUTH_TOKEN)",
  ...
  "startup.subtitle": "Проверьте, сможет ли Codex подключиться к hubapi после перезагрузки, прежде чем локальный прокси вызовет бесконечное переподключение.",
  ...
  "startup.summary.protected": "hubapi будет доступен после перезагрузки",
  ...
  "startup.riskDetailCustomLocal": "Codex направлен на пользовательский локальный шлюз. hubapi не может управлять или проверять его перезапуск.",
  ...
  "startup.codexRuntime.clampHidden": "Некоторые уровни рассуждений скрыты, потому что hubapi использует Codex {version}.",
  "startup.codexRuntime.clampHiddenWithEfforts": "Некоторые уровни рассуждений скрыты, потому что hubapi использует Codex {version} (удалены: {efforts}).",
  "startup.codexRuntime.olderBinary": "hubapi использует более старый бинарник Codex ({version}). Доступна более новая установка.",

  // dashboard
  "dash.subtitle": "Актуальное состояние локального прокси hubapi, его провайдеров и моделей, маршрутизируемых в Codex.",
  ...
  "dash.routes.title": "Недавние данные маршрутов",
  "dash.routes.hint": "Только записанные запросы; предпросмотр конфигурации отделён.",
  "dash.routes.viewAll": "Открыть логи",
  "dash.routes.unavailable": "Сохранённая история запросов недоступна.",
  "dash.routes.empty": "Сохранённых запросов пока нет.",
  "dash.routes.unknown": "Трасса маршрута недоступна",
  "dash.routes.attempts": "Попыток: {count}",
  ...
  "dash.codexAutoStart": "Запускать hubapi вместе с Codex",
  ...
  "dash.shadowCallTooltip": "Codex App в фоновом режиме вызывает служебную модель для генерации заголовков тредов, сообщений коммитов и оркестрации навыков. Эта модель менялась между версиями клиента, поэтому hubapi перехватывает весь набор: {models}. Включите функцию, чтобы перенаправлять такие вызовы на выбранную вами модель.",
  ...
  "dash.maintenanceHint": "Обновите каталог моделей Codex или установите более новую версию hubapi.",
  ...
  "dash.projectConfigTitle": "Конфигурация Codex в проекте обходит hubapi",
  "dash.projectConfigHint": "Эти локальные настройки репозитория переопределяют прокси hubapi (например, направляют запросы напрямую в OpenCode Go). Удалите их, чтобы в этом проекте действовала маршрутизация из ~/.codex/config.toml.",
  "dash.checkUpdate": "Проверить обновления",
  "dash.updateTitle": "Обновление hubapi",
  ...
  "dash.updateStatus.running": "Обновление hubapi.",
  ...
  // providers
  "prov.subtitle": "Настройте вышестоящих провайдеров, которых hubapi маршрутизирует в Codex. Войдите в аккаунт, добавьте провайдера или отредактируйте конфигурацию вручную.",
  ...
  "oauthTos.anthropicBody": "Прямое повторное использование OAuth-токенов подписки Claude через сторонний прокси, такой как hubapi, не является поддерживаемой интеграцией Anthropic и может привести к ограничению доступа. Поддерживаемые интеграции Agent SDK, использующие подписки Claude, — это отдельный механизм.",
  "oauthTos.highBody": "hubapi подключает {provider} через сторонний механизм OAuth. Неподдерживаемое использование может привести к ограничению или приостановке доступа.",
  "oauthTos.elevatedBody": "hubapi подключает {provider} через неофициальный механизм OAuth. По возможности используйте официальный клиент; нетипичный или автоматизированный трафик может быть расценён как злоупотребление, и доступ может быть ограничен или приостановлен.",
  "oauthTos.saferPath": "Более безопасный вариант: вместо этого настройте API-ключ в hubapi.",
  ...
  "models.subtitle": "Управляйте тем, какие модели видит Codex — нативные GPT (сквозной проброс) и модели маршрутизируемых провайдеров, сгруппированные по провайдеру (нажмите на заголовок, чтобы свернуть группу). Скрытые модели исчезают из каталога и селектора, но остаются вызываемыми по точному id. Изменения применяются на следующем ходе Codex — hubapi сбрасывает 5-минутный кэш моделей Codex, поэтому перезапуск не требуется.",
  ...
  "logs.subtitle": "Сохранённые запросы через локальный прокси hubapi, новые сверху.",
  "logs.history.indexed": "Проиндексировано сохранённых запросов: {count}",
  "logs.history.repaired": "Индекс истории восстановлен",
  "logs.history.loadOlder": "Загрузить более старые запросы",
  "logs.history.loadError": "Не удалось загрузить старые запросы.",
  ...
  "logs.filter.advanced": "Фильтры сохранённой истории",
  "logs.filter.provider": "ID провайдера",
  "logs.filter.model": "Конечная модель",
  "logs.filter.requestedModel": "Запрошенная модель",
  "logs.filter.status": "Статус HTTP",
  "logs.filter.protocol": "Входящий протокол",
  "logs.filter.apiKey": "ID ключа API",
  "logs.filter.profile": "ID профиля маршрута",
  "logs.filter.fallback": "Использован fallback",
  "logs.filter.fallback.any": "Любой",
  "logs.filter.fallback.yes": "Да",
  "logs.filter.fallback.no": "Нет",
  "logs.filter.apply": "Применить фильтры",
  "logs.filter.clearAll": "Очистить всё",
  ...
  "logs.detail.route.loading": "Загрузка записанных данных маршрута…",
  "logs.detail.route.loadError": "Данные маршрута не загрузились; маршрут не выводится из конечного провайдера.",
  "logs.detail.route.requested": "Запрошенная модель",
  "logs.detail.route.final": "Конечная цель",
  "logs.detail.route.outcome": "Результат",
  ...
  "integrations.subtitle": "Подключайте клиенты к hubapi, управляйте учётными данными и восстанавливайте конфигурацию клиентов.",
  ...
  "integrations.codex.body": "Подключением Codex управляет прокси-сервис. При запуске hubapi оно применяется, а при остановке сервиса восстанавливается нативная маршрутизация.",
  ...
  "integrations.dialog.desktop.changes": "Если {path} содержит профиль шлюза, управляемый hubapi, Desktop сначала выберет новый стандартный профиль без учётных данных, а затем удалит старый профиль и резервную копию.",
  "integrations.dialog.desktop.breakage": "Claude Desktop вернётся к обычному Claude вместо моделей, маршрутизируемых через hubapi.",
  "integrations.dialog.desktop.undo": "При повторном включении профиль hubapi будет создан заново из сохранённых назначений моделей.",
  ...
  "integrations.native.error.desktopCleanupIncomplete": "Claude Desktop указывает на стандартный режим, но старые файлы учётных данных hubapi остались в: {paths}.",
  ...
  "integrations.detail.grokAbsent": "В конфигурации нет блока hubapi",
  "integrations.dialog.grok.title": "Отключить интеграцию Grok Build?",
  "integrations.dialog.grok.changes": "Из {path} будет удалён только блок, отмеченный hubapi. Содержимое, добавленное вручную вне блока, останется без изменений.",
  "integrations.dialog.grok.breakage": "После отключения псевдонимы моделей hubapi исчезнут из Grok Build. Модели, использовавшиеся с учётной записью xAI, останутся доступны.",
  "integrations.dialog.grok.undo": "Если hubapi запущен на loopback-адресе, при повторном включении будет записан новый блок из списка доступных на тот момент моделей.",
  "integrations.dialog.grok.confirm": "Отключить",
  "integrations.native.msg.nonLoopbackRemoved": "Grok Build можно регистрировать автоматически, только когда hubapi запущен на loopback-адресе. Предыдущий блок, указывавший на loopback-адрес, удалён.",
  "integrations.native.msg.nonLoopbackRemovedNoop": "Grok Build можно регистрировать автоматически, только когда hubapi запущен на loopback-адресе. Предыдущего блока для удаления не было.",
  "integrations.native.msg.nonLoopbackSuperseded": "Grok Build можно регистрировать автоматически, только когда hubapi запущен на loopback-адресе. Тем временем другая программа записала в конфигурацию новый блок, поэтому текущий блок в файле создан не этим запросом.",
  "integrations.native.error.orphanedMarker": "В {path} есть начальная метка hubapi, но нет конечной. Файл не изменён, потому что невозможно надёжно определить конец блока.",
  ...
  "integrations.onboarding": "При применении сначала сохраняется резервная копия, а затем записывается один блок провайдера hubapi. При отключении удаляется только этот блок, а сохранённый снимок можно восстановить.",
  "integrations.empty.title": "Установленные клиенты не обнаружены",
  "integrations.empty.body": "Установите поддерживаемый клиент, затем вернитесь сюда и примените hubapi.",
  ...
  "integrations.error.conflict": "После записи hubapi конфигурация изменилась. Ничего не было удалено.",
  ...
  "integrations.bulk.body": "Будет удалён только блок, принадлежащий hubapi. Для каждого клиента предварительно сохраняется снимок исходного состояния.",
  ...
  "integrations.semantics.dsh": "hubapi управляет только llm-pi-ai.providers.opencodex в $DSH_HOME/settings.yaml. DSH применяет этот провайдер горячей перезагрузкой; модель по умолчанию и deepseek-official остаются без изменений. Сейчас поддерживается только loopback; реальные учётные данные не записываются.",
  ...
  "accountPool.strategyDesc": "Как hubapi назначает аккаунт новой/непривязанной задаче.",
  ...
  // api access page
  "api.title": "Прокси API",
  "api.subtitle": "Сгенерированные API-ключи дают внешним приложениям доступ к прокси hubapi. Аутентификация — через заголовок {authHeader}; какие заголовки принимает каждый эндпоинт, смотрите в таблице ниже.",
  "api.proxy.title": "Единый прокси-шлюз",
  "api.proxy.online": "В сети и принимает API-запросы",
  "api.proxy.flowLabel": "Поток прокси-запросов",
  "api.proxy.clients": "Клиенты",
  "api.proxy.providers": "Провайдеры",
  "api.proxy.baseUrl": "Базовый URL",
  "api.proxy.protocols": "Входящие протоколы",
  "api.proxy.auth": "Аутентификация",
  ...
  "claude.smallFastModelNativeWarning": "Если модель не выбрана, hubapi не задаёт переопределения вспомогательной модели. Claude Code может использовать нативную модель Sonnet, что может привести к расходам у вашего нативного провайдера.",
  ...
  "grok.subtitle": "Модели, зарегистрированные hubapi в вашей конфигурации Grok.",
  ...
  "grok.notConfiguredHint": "Установите Grok и перезапустите прокси — hubapi запишет управляемый блок в:",
  ...
  "cws.howBody": "Запросите у Codex публичное имя модели комбо. Если оно не задано, используется combo/<id>. hubapi выбирает цель и переключается на следующую только при сбоях вышестоящего провайдера, допускающих повтор. Если доступных целей не осталось, запрос завершается ошибкой, а не переходит на глобальный провайдер по умолчанию.",
