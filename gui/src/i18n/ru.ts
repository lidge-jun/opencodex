...
export const ru: Record<TKey, string> = {
  ...
  "codexAuth.resetCliOnly": "Кредит сброса можно использовать только в loopback-панели. Для удалённой панели используйте локальный CLI-процесс согласия на хосте OpenCodex.",
  "codexAuth.confirmResetTitle": "Использовать кредит сброса?",
  "codexAuth.confirmResetDesc": "Текущие лимиты запросов будут мгновенно сброшены. После подтверждения повторно введите токен администратора OpenCodex, чтобы подтвердить согласие владельца. У вас осталось кредитов: {count}.",
  ...
  "codexAuth.resetHistoryFull": "Журнал операций reset credit заполнен. Перед новой попыткой попросите сопровождающего увеличить ёмкость или применить утверждённую политику удаления истории.",
