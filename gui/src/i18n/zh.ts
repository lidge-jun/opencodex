...
export const zh: Record<TKey, string> = {
  ...
  "codexAuth.namespacesTitle": "在模型选择器中分别显示每个 Codex 账户",
  "codexAuth.namespacesOnDesc": "每个可用的 Codex 账户都会单独显示一份各 GPT 模型。Main 表示内置登录；每个公开的选择器标签都会在内部映射到一个已存储账户。已添加账户会使用用户指定的显示名称；如果未设置显示名称，则使用稳定且保护隐私的 pXXXXXX 标签。存储的账户 ID 和电子邮件不会被用作标签。现有标签和自定义标签保持不变。选择后，对话会固定使用该账户；OpenCodex 不会切换或回退到其他账户。",
  "codexAuth.namespacesOffDesc": "如果你有多个 Codex 账户，并希望无需退出登录即可明确选择对话使用的账户，请开启此项。关闭后只会隐藏这些选项，不会删除任何账户。",
  "codexAuth.namespacesCompatibilityNote": "这会用明确的账户专属选项替换模型选择器中的常规 GPT 项。现有对话、自定义账户标签和已保存的模型选择会保持当前路由，不带前缀的 GPT 模型 ID 会继续遵循原有的账户池或直接模式。",
  "codexAuth.namespacesSaved": "模型选择器中的账户显示已更新。如果列表仍旧，请重新打开 Codex。",
  "codexAuth.namespacesSaveFailed": "无法更新模型选择器。之前的设置保持不变。",
  "codexAuth.add": "添加",
  ...
  "codexAuth.catalogRefreshPending": "账号更改已保存，但模型选择器未能刷新。请运行 `ocx sync` 重试。",
