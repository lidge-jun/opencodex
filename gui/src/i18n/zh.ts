...
export const zh: Record<TKey, string> = {
  ...
  "codexAuth.resetCliOnly": "仅可从环回地址的仪表板使用重置额度。使用远程仪表板时，请改在 OpenCodex 主机上运行本地 CLI 同意流程。",
  "codexAuth.confirmResetTitle": "使用重置额度？",
  "codexAuth.confirmResetDesc": "这将立即重置您当前的使用限制。确认后，请重新输入 OpenCodex 管理员令牌以证明所有者同意。剩余额度：{count} 个。",
  ...
  "codexAuth.resetHistoryFull": "重置额度操作历史已满。再次尝试前，请让维护者扩展容量或应用已批准的历史清理策略。",
