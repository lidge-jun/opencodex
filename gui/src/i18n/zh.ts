...
export const zh: Record<TKey, string> = {
  ...
  "codexAuth.namespacesTitle": "在模型选择器中显示 Codex 账户",
  "codexAuth.namespacesOnDesc": "选择器会为每个已添加的 Codex 账户分别显示每个 GPT 模型。选择带账户名称的模型后，整个线程都会使用该账户的额度，并且不会切换账户。",
  "codexAuth.namespacesOffDesc": "GPT 模型遵循上方的账户池或直接模式。需要为线程选择确切账户时请开启。",
  "codexAuth.namespacesCompatibilityNote": "这只会改变新模型选项的显示方式，不会更改当前账户池账户、现有线程或已保存设置。",
  "codexAuth.namespacesSaved": "账户选择已更新。如果列表仍旧，请重新打开 Codex。",
  "codexAuth.namespacesSaveFailed": "无法更新账户选择。之前的设置保持不变。",
  "codexAuth.add": "添加",
