...
export const zh: Record<TKey, string> = {
  ...
  "codexAuth.namespacesOnDesc": "每个可用的 Codex 账户都会单独显示一份各 GPT 模型。已添加账户的标签由其卡片上显示的本地 ID 生成，内置登录的标签以 Main 开头；如有重名则添加数字后缀。选择后，对话会固定使用该账户；OpenCodex 不会切换或回退到其他账户。",
  "codexAuth.namespacesOffDesc": "如果你有多个 Codex 账户，并希望无需退出登录即可明确选择对话使用的账户，请开启此项。关闭后只会隐藏这些选项，不会删除任何账户。",
  "codexAuth.namespacesCompatibilityNote": "这会用明确的账户专属选项替换模型选择器中的常规 GPT 项。现有对话和已保存的模型选择会保持当前路由，不带前缀的 GPT 模型 ID 会继续遵循原有的账户池或直接模式。",
