...
"common.discard": "丢弃",
  ...
  "compactionRouting.providerWarningScoped": "启用此设置后，仅来自 {sources} 的压缩请求会将完整对话内容发送给 {provider} 进行摘要；其他对话仍使用自身模型压缩。",
  "compactionRouting.comboWarningScoped": "启用此设置后，仅来自 {sources} 的压缩请求会将完整对话内容发送给组合 {combo} 进行摘要；其他对话仍使用自身模型压缩。该组合会按顺序尝试其目标（{providers}），并使用第一个响应的目标，因此其中任意一个都可能收到这些对话。",
