...
- **한 번 로그인하면 API 키는 생략.** Command Code 계정 로그인과 xAI, Anthropic, Kimi OAuth를 지원하므로 기존 계정으로 인증할 수 있습니다. 또는 `codex login`을 forward 하거나, API 키를 붙여넣거나, `${ENV_VAR}` 참조를 쓸 수 있습니다 — 선택은 자유입니다.
...
| Command Code | `command-code` | oauth / 기존 CLI 로그인 가져오기 |
