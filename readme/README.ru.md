...
- **Веб-поиск и vision-сайдкары.** Веб-поиск использует OpenAI/Anthropic; vision также поддерживает
  Chat-бэкенд через настроенного провайдера (`provider/model`). См. [руководство по сайдкарам](https://opencodex.me/guides/sidecars/).
