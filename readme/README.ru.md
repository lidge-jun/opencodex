...
В дашборде появилась пригласительная страница **Remote** для центрального MVP
`opencodexpages.me`. Настройка начинается в локальном `ocx gui`: подтвердите ПК через GitHub,
задайте отдельный пароль Remote и hostname, затем отслеживайте provisioning Tunnel/Agent.
OAuth-токены GitHub не сохраняются и не передаются на ПК. Подписанный Linux helper installer с
правами администратора и помощники Windows/macOS ещё не поставляются; это не общий релиз и
**Super Sync в него не входит**.

Приведённая ниже LAN-привязка — отдельный самостоятельно управляемый вариант без центрального Remote.
