...
Compared to `vendor/main` after FF to `upstream/main`.
Tool: `git log vendor/main..overlay` plus `gh pr view`.

`git log vendor/main..overlay` must stay the curated overlay (today: one `fork:` commit), not this 65-commit mix.
...
Do not fold into `overlay`. Replay the GitHub `feat/*` PR heads onto `run/main` if you need them locally; local `feat/*` branches are not required.
...
- `run/main` not built (would replay open feat stacks).
- No push.
