...
export const PROVIDER_REGISTRY: readonly ProviderRegistryEntry[] = [
  ...
  },
  {
    ...
    note: "Experimental Cursor bridge. Live transport + model discovery after standalone PKCE login (`ocx login cursor`). Native read/write/delete/shell/fetch defaults to codex-sandbox (auto when the request declares Codex danger-full-access); set providers.cursor.nativeLocalExec to \"on\" (always), \"off\" (never), or \"codex-sandbox\" (declared sandbox only). That claim is caller-controlled prose the proxy cannot verify, and the auth-free loopback admits any process on this host (incl. other local users) — only for trusted data-plane clients. Legacy unsafeAllowNativeLocalExec: true ≡ \"on\". Config: ~/.opencodex/config.json (Providers → Cursor → Edit JSON).",
