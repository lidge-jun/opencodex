 ...
 * third-party live `/v1/models` endpoint, so live discovery instead reads the account-scoped
 * `--help` roster (src/adapters/codebuddy/live-models.ts) and this static catalog is only the
 * degraded seed for installs where the CLI is missing or the roster line cannot be parsed. The
 * bundled manifest lags the server-side roster — an account's `--help` line can list models the
 * manifest does not know — so a mismatch between this file and a live client is expected, not a
 * catalog bug.
 */
