...
/** Merge registry verbosity defaults per model while preserving explicit user values. */
function applyVerbosityDefaults(
  prov: OcxProviderConfig,
  defaults: Readonly<Record<string, boolean>> | undefined,
): void {
  if (!defaults) return;
 ...
 *
 * Deliberately narrow: only the per-model catalog/wire capability maps, and only via
 ...
 */
function enrichModelCapabilitiesByDestination(prov: OcxProviderConfig): void {
  ...
  applyVerbosityDefaults(prov, destination?.modelSupportsVerbosity);
}
...
export function enrichProviderFromRegistry(name: string, prov: OcxProviderConfig): void {
  ...
  if (!entry || !providerMatchesRegistryTransportWithStaticGuards(name, prov)) {
    ...
    enrichModelCapabilitiesByDestination(prov);
  ...
  applyVerbosityDefaults(prov, entry.modelSupportsVerbosity);
