...
export const EXPECTED_PRICE_OVERLAYS: readonly ExpectedPriceOverlay[] = [
  // claude-fable-5-1 now has a nonzero jawcode row for the Anthropic bundle, so jawcode is
  // canonical for both Anthropic surfaces (the bundle collapses anthropic-apikey onto
  // anthropic). Keep these shipped overlay rows for compatibility and direct overlay lookups;
  // resolveMatchedPrice intentionally gives the jawcode row precedence.
