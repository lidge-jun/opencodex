...
import { MAIN_CODEX_ACCOUNT_ID, MainAuthJsonChangedDuringRefreshError } from "../../codex/main-account";
...
export function nativeMainRefreshFailureResponse(error: unknown): Response {
  if (error instanceof TokenRefreshError && error.reason !== "unknown") {
...
export function mapCodexAuthContextErrorToResponse(
  ...
  if (error instanceof TokenRefreshError || error instanceof MainAuthJsonChangedDuringRefreshError) {
    return nativeMainRefreshFailureResponse(error);
  }
  return undefined;
}
