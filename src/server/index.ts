...
export function warnAgentTaskRecoveryStartup(config: {
  ...
  console.warn("?ʕ?  Experimental encrypted V2 task recovery is enabled.");
...
export function startServer(port?: number, deps: StartServerDeps = {}): Server<WsData> {
  ...
  if (loopbackServer) {
    ...
    console.warn(`?ʕ?  Unauthenticated loopback listener active on http://127.0.0.1:${loopbackPort}`);
