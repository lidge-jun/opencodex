...
export async function forwardGoWebSocketFrames(baseUrl: string, requestToken: string, frame: Record<string, unknown>, admission: unknown): Promise<string[]> {
  ...
  return await new Promise<string[]>((resolve, reject) => {
    const socket = net.createConnection({ host: url.hostname, port: Number(url.port) }); const key = randomBytes(16).toString("base64"); let buffer = Buffer.alloc(0); let upgraded = false; const frames: string[] = [];
