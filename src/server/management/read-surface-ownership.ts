...
const lab = "Compatibility Lab remains a last-migrated subsystem. Its local SQLite transport and optional activation state move with the Go serving binary at the runtime flip, rather than creating a second Lab implementation in the sidecar.";
...
export const READ_SURFACE_DIFF_MATRIX: readonly ReadSurfaceDiffMatrixEntry[] = [
  ...
  ...deferred([
    ...
    { module: "server/management/lab-automation-routes", stateSources: ["disk", "serving-process"], rationale: lab, routes: [["GET", "/api/lab/automation"], ["GET", "/api/lab/automation/runs"]] },
    { module: "server/management/lab-routes", stateSources: ["disk", "serving-process"], rationale: lab, routes: [["GET", "/api/lab/artifacts"], ["GET", "/api/lab/catalog"], ["GET", "/api/lab/events"], ["GET", "/api/lab/observations"], ["GET", "/api/lab/production-signals"], ["GET", "/api/lab/public/community"], ["GET", "/api/lab/status"], ["GET", "/api/lab/subjects"], ["GET", "/api/lab/verdicts"], ["GET", "/api/lab/subjects/{id}"], ["GET", "/api/lab/events/{id}"], ["GET", "/api/lab/artifacts/{digest}"]] },
    { module: "server/management/logs-usage-routes", stateSources: ["disk", "serving-process"], rationale: flip, routes: [["GET", "/api/claude/inbound-debug"], ["GET", "/api/debug"], ["GET", "/api/debug/injection-logs"], ["GET", "/api/debug/logs"], ["GET", "/api/debug/usage-logs"], ["GET", "/api/logs"], ["GET", "/api/storage/cleanup-policy"], ["GET", "/api/storage/cleanup-policy/test-stream"], ["GET", "/api/storage/trash"], ["GET", "/api/storage/trash/restore/test-stream"], ["GET", "/api/usage"]] },
    { module: "server/management/model-routes", stateSources: ["disk", "serving-process", "external-state"], rationale: flip, routes: [["GET", "/api/aliases"], ["GET", "/api/catalog"], ["GET", "/api/client-config"], ["GET", "/api/model-discovery"], ["GET", "/api/model-presets"], ["GET", "/api/models"], ["GET", "/api/selected-models"]] },
