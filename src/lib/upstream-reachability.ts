 ...
 *
 * Bun 1.3.14 collapses DNS failure and TCP refusal into one label class
 * (`ConnectionRefused` / `FailedToOpenSocket`, errno 0, no cause) and alternates
 * between the two labels as its DNS cache is evicted; Node's undici emits the
 * classic `ECONNREFUSED` / `ENOTFOUND` / `EAI_AGAIN` / `ENETUNREACH` /
...
/** Host component of a transport failure key, or null when the URL is malformed. */
export function transportFailureHost(url: string): string | null {
  try {
