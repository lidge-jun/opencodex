 ...
 *
 * A scoped request (`options.liveRosterScope`) reads only that credential's
 * recorded evidence: an absent scope slot is an empty set, never the global
 * slot — account isolation is the point of the scope.
 */
export function resolveCursorSelection(
  ...
  const evidence = liveMaxModeIds
    ?? (options.liveRosterScope
      ? liveCursorMaxModeBasesByScope.get(options.liveRosterScope) ?? EMPTY_LIVE_MAX_MODE_EVIDENCE
...
const EMPTY_LIVE_MAX_MODE_EVIDENCE: ReadonlySet<string> = new Set();

/** Register a credential scope under its provider so provider-scoped clears reach every record kind. */
function registerLiveCursorRosterScope(scope: { provider: string; key: string }): void {
...
/** Provider names that currently hold credential-scoped roster state. */
export function liveCursorRosterScopedProviders(): ReadonlySet<string> {
  return new Set(liveCursorRosterScopesByProvider.keys());
}
...
export function recordLiveCursorClaudeModels(liveIds: readonly string[], scope?: { provider: string; key: string }): void {
  ...
  if (scope) {
    ...
    registerLiveCursorRosterScope(scope);
  } else {
...
export function recordLiveCursorMaxModeModels(liveIds: readonly string[], scope?: { provider: string; key: string }): void {
  ...
  if (scope) {
    liveCursorMaxModeBasesByScope.set(scope.key, bases);
    registerLiveCursorRosterScope(scope);
  } else {
