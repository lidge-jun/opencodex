...
export function buildCursorToolGuidanceSystemNote(
  ...
  const notes = [
    ...
    hasBareExec
      ? `If a Cursor-native file read, directory listing, grep, or shell operation is rejected by the runtime, retry through ${shellBridgeLabel} with an equivalent host-shell-safe command (POSIX: \`cat\`/\`ls\`/\`rg\`; Windows PowerShell: \`Get-Content\`/\`Get-ChildItem\`/\`Select-String\`). That rejection means native local exec is off; the bridge remains available. For file edits, use ${structuredEditNames.length > 0 ? `the structured edit tools (${quotedNames(structuredEditNames)}) or ` : ""}\`apply_patch\` when available.`
      : undefined,
