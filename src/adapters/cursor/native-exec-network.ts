...
const NATIVE_FETCH_DISABLED =
  "Cursor-native Fetch is not enabled for this request (`nativeLocalExec` is off). Equivalent network work is available through the Codex shell bridge tool `shell_command` (aliases: `exec_command`, `mcp_opencodex-responses_shell_command`, `mcp_opencodex-responses_exec_command`) using curl or wget. This is a routing fallback to the bridge, not a missing capability — call that tool to continue.";
