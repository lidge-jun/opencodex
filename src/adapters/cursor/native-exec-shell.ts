...
import { nativeShellDisabledMessage as catalogNativeShellDisabledMessage, type CursorNativeExecBridgeCatalog } from "./native-exec-bridge";
import {
  createAdmissionGate,
  type AdmissionLease,
...
export function nativeShellDisabledMessage(catalog?: CursorNativeExecBridgeCatalog): string {
  return catalogNativeShellDisabledMessage(catalog);
}

function rejectedShellResult(command: string, cwd: string, started: number, catalog?: CursorNativeExecBridgeCatalog) {
  return create(ShellResultSchema, {
    result: {
      ...
      value: create(ShellFailureSchema, {
        ...
        signal: "",
        stdout: "",
        stderr: nativeShellDisabledMessage(catalog),
...
export function rejectShellExecForPolicy(execMsg: ExecServerMessage, catalog?: CursorNativeExecBridgeCatalog): Uint8Array {
  if (execMsg.message.case !== "shellArgs") throw new Error("invalid shell exec");
  const args = execMsg.message.value;
  return execBytes(execMsg, "shellResult", rejectedShellResult(args.command, resolve(args.workingDirectory || process.cwd()), Date.now(), catalog));
}
...
export function rejectShellStreamExecForPolicy(execMsg: ExecServerMessage, catalog?: CursorNativeExecBridgeCatalog): Uint8Array[] {
  ...
  return [
    ...
    execBytes(execMsg, "shellStream", create(ShellStreamSchema, {
      event: { case: "stderr", value: create(ShellStreamStderrSchema, { data: nativeShellDisabledMessage(catalog) }) },
    })),
    execBytes(execMsg, "shellStream", create(ShellStreamSchema, {
      event: { case: "exit", value: create(ShellStreamExitSchema, { code: 1, cwd, aborted: true }) },
    })),
    execBytes(execMsg, "shellResult", rejectedShellResult(args.command, cwd, started, catalog)),
...
export function rejectBackgroundShellSpawnExecForPolicy(execMsg: ExecServerMessage, catalog?: CursorNativeExecBridgeCatalog): Uint8Array {
  ...
  return execBytes(execMsg, "backgroundShellSpawnResult", create(BackgroundShellSpawnResultSchema, {
    result: { case: "error", value: create(BackgroundShellSpawnErrorSchema, { command: args.command, workingDirectory: cwd, error: nativeShellDisabledMessage(catalog) }) },
  }));
}
...
export function rejectWriteShellStdinExecForPolicy(execMsg: ExecServerMessage, catalog?: CursorNativeExecBridgeCatalog): Uint8Array {
  ...
  return execBytes(execMsg, "writeShellStdinResult", create(WriteShellStdinResultSchema, {
    result: { case: "error", value: create(WriteShellStdinErrorSchema, { error: nativeShellDisabledMessage(catalog) }) },
  }));
}
