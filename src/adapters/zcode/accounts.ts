...
export interface ZcodeAccount { id: string; label: string; subjectHash?: string }
...
export function readAccount(id: string): ZcodeAccount {
  ...
  try {
    ...
    if (value.id !== id || typeof value.label !== "string" || value.label.length > 80
      || (value.subjectHash !== undefined && !/^[a-f0-9]{64}$/.test(value.subjectHash))) throw new Error();
...
export function writeAccount(account: ZcodeAccount): void {
  ...
  if (!label || label.length > 80 || /[\x00-\x1f]/.test(label)
    || (account.subjectHash !== undefined && !/^[a-f0-9]{64}$/.test(account.subjectHash))) throw new Error("account_invalid");
...
export function allocateAccount(label: string): ZcodeAccount {
  if (listAccounts().length >= 20) throw new Error("account_limit");
  const account = { id: randomUUID(), label };
