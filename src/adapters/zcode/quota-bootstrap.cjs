// Runs only inside the quota sandbox. Native host and RPC modules are unmodified official ZCode.
...
fs.mkdirSync(process.env.HOME+'/.zcode/v2',{recursive:true});
const input=JSON.parse(fs.readFileSync('/desktop/config.json','utf8'));
if(process.argv[2]==='advanced') {
 ...
 fs.writeFileSync(process.env.HOME+'/.zcode/v2/config.json',JSON.stringify({provider:{'builtin:zai-coding-plan':{...raw,name:'Z.ai - Coding Plan',enabled:true}}}),{mode:0o600});
} else {
 fs.copyFileSync('/desktop/config.json',process.env.HOME+'/.zcode/v2/config.json');
 if(fs.existsSync('/desktop/credentials.json')) fs.copyFileSync('/desktop/credentials.json',process.env.HOME+'/.zcode/v2/credentials.json');
}
setTimeout(()=>{originalLog(JSON.stringify({error:'timeout'}));process.exit(1)},40000);
(async()=>{
 const dir='/zcode/resources/app.asar/out/host';
 let classes;
