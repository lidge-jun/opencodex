...
import { writePublicEvidenceBundle } from "./store";
import {
...
export type PublicVerificationSummaryV1 =
  ...
  | {
    status: Exclude<PublicBundleVerificationResult["status"], "cryptographically_valid">;
    locallyVerified: false;
    detail?: string;
  };
...
function utcDay(timestamp: number): string {
  ...
  if (!Number.isFinite(date.getTime())) {
    throw new PublicEvidenceValidationError(
      ...
      "selected observation has an invalid completion timestamp",
    );
  }
...
function canonicalVerdictForObservation(
  ...
  const verdict = page.items.find((row) =>
    ...
    && row.contributingEventIds.includes(observation.eventId),
  );
  return verdict?.verdict ?? null;
}

/**
 * Build a public-safe unsigned bundle from explicitly selected local Lab events.
 *
 * This function is read-only. It does not create a publisher key, write an
 * export, import community evidence, contact a provider, or perform network IO.
 * Live-route/task records remain fail-closed unless a future trusted caller can
 * provide an internally-derived public authority; this operator currently never
 * accepts authority assertions from user input.
 */
export function previewLocalPublicEvidence(
  ...
  for (const eventId of eventIds) {
    ...
    latestObservationCompletedAt = Math.max(
      latestObservationCompletedAt ?? event.completedAt,
      event.completedAt,
    );
  ...
  if (latestObservationCompletedAt === null) {
    throw new PublicEvidenceValidationError(
      "public_selection_empty",
      "public evidence selection contains no observation events",
    );
  }
  ...
  for (const row of projected.excluded) {
    excluded.push({
      eventId: projectEventIds[row.index]!,
      reason: row.reason,
    });
  }
...
/** Explicit local export. No remote publish transport exists in CL-10.1..10.4. */
export function exportLocalPublicEvidence(
  ...
  if (preview.bundle.records.length === 0) {
    throw new PublicEvidenceValidationError(
      "public_export_empty",
      "selected events produced no exportable public evidence records",
    );
  }
  const signer = getOrCreatePublicPublisher(configDir);
  const bundle = signPublicEvidenceBundle(preview.bundle, signer);
  const stored = writePublicEvidenceBundle(bundle, configDir);
  return { bundle, stored, excluded: preview.excluded };
}
...
export function summarizePublicEvidenceVerification(raw: unknown): PublicVerificationSummaryV1 {
  ...
  return {
    ...
    publisherKeyId: verified.bundle.publisher.keyId,
    locallyVerified: false,
  };
}
...
function readBoundedPublicFile(path: string): Buffer {
  ...
  if (stats.size > MAX_PUBLIC_FILE_BYTES) {
    throw new PublicEvidenceValidationError(
      ...
      "public evidence input exceeds 2 MiB",
    );
  }
...
function parsePublicFile(path: string): unknown {
  ...
  try {
    ...
    throw new PublicEvidenceValidationError(
      "public_file_json",
      "public evidence input is not valid JSON",
    );
  }
}
