...
function bundleObjectPath(publisherKeyId: string, bundleId: string, configDir?: string): string {
  return join(
    ...
    `bundle-${assertId(publisherKeyId)}-${assertId(bundleId)}.json`,
  );
}
  ...
  try {
    ...
    if (!readBounded(path).equals(bytes)) {
      throw new PublicEvidenceValidationError(
        "community_conflict",
        `${kind} identity already exists with different bytes`,
      );
    }
...
function readVerifiedBundleAt(path: string): PublicEvidenceBundleV1 {
  ...
  if (verified.status !== "cryptographically_valid") {
    throw new PublicEvidenceValidationError(
      verified.status,
      verified.detail ?? "stored community bundle invalid",
    );
  }
...
export function importCommunityEvidenceBundle(
  ...
  if (verified.status !== "cryptographically_valid") {
    throw new PublicEvidenceValidationError(
      verified.status,
      verified.detail ?? "community bundle verification failed",
    );
  }
  validateCommunityEvidenceAuthorities(verified.bundle);
  ensureLabDirs(configDir);
  const stored = persistAt(
    bundleObjectPath(verified.bundle.publisher.keyId, verified.bundle.bundleId, configDir),
    "bundle",
    verified.bundle,
  );
  return {
...
export function readCommunityEvidenceBundleForPublisher(
  ...
  if (bundle.bundleId !== bundleId || bundle.publisher.keyId !== publisherKeyId) {
    throw new PublicEvidenceValidationError(
      ...
      "stored community bundle does not match filename identity",
    );
  }
  return bundle;
}
...
export function readCommunityEvidenceBundle(bundleId: string, configDir?: string): PublicEvidenceBundleV1 {
  assertId(bundleId);
  const matches = files(configDir).flatMap((name) => {
    const match = COMMUNITY_BUNDLE_FILE_RE.exec(name);
    return match && match[2] === bundleId ? [{ name, publisherKeyId: match[1]! }] : [];
  });
  if (matches.length === 0) {
    throw new PublicEvidenceValidationError("community_missing", "community bundle not found");
  }
  if (matches.length !== 1) {
    throw new PublicEvidenceValidationError(
      "community_ambiguous_bundle",
      "bundleId is shared by multiple publishers; publisherKeyId is required",
    );
  }
  return readCommunityEvidenceBundleForPublisher(bundleId, matches[0]!.publisherKeyId, configDir);
}
...
function findTargetBundle(revocation: unknown, configDir?: string): PublicEvidenceBundleV1 {
  ...
  const candidates = allCommunityBundles(configDir).filter(
    (bundle) => bundle.publisher.keyId === publisherKeyId,
  );
...
export function importCommunityEvidenceRevocation(
  ...
  if (verified.status !== "cryptographically_valid") {
    throw new PublicEvidenceValidationError(
      verified.status,
      verified.detail ?? "community revocation verification failed",
    );
  }
  ensureLabDirs(configDir);
  const stored = persistAt(
  ...
  return {
    ...stored,
    status: "cryptographically_valid",
    revocationId: verified.revocation.revocationId,
  };
}

function verifiedRevocationsForBundle(
...
export function listCommunityEvidence(configDir?: string): CommunityEvidenceSummaryV1[] {
  return allCommunityBundles(configDir).map((bundle) => {
    ...
    for (const revocation of verifiedRevocationsForBundle(bundle, configDir)) {
      if (revocation.targets.some(
        (target) => target.kind === "bundle" && target.id === bundle.bundleId,
      )) {
    ...
    a.bundleId.localeCompare(b.bundleId) || a.publisherKeyId.localeCompare(b.publisherKeyId));
}
