...
function assertString(value: unknown, field: string, max = MAX_STRING_BYTES): string {
  if (typeof value !== "string") {
    throw new PublicEvidenceValidationError("invalid_type", `${field} must be string`);
  }
  if (value.includes("\0")) {
    throw new PublicEvidenceValidationError("nul_forbidden", `${field} contains NUL`);
  }
...
function assertPublicToken(value: unknown, field: string, max = 256): string {
  ...
  if (!/^[A-Za-z0-9][A-Za-z0-9._:/+-]*$/.test(text)) {
    throw new PublicEvidenceValidationError("unsafe_string", `${field} contains unsupported characters`);
  }
  return text;
}
...
function assertAdapter(value: unknown, field: string): PublicAdapterFamilyV1 {
  ...
  if (!(PUBLIC_ADAPTER_FAMILIES as readonly string[]).includes(text)) {
    throw new PublicEvidenceValidationError("closed_set", `${field} is not a public adapter family`);
  }
...
function assertDay(value: unknown, field: string): string {
  ...
  if (!Number.isFinite(parsed.getTime()) || parsed.toISOString().slice(0, 10) !== text) {
    throw new PublicEvidenceValidationError("invalid_day", `${field} is not a UTC calendar day`);
  }
  return text;
}
...
function validateIncidentRefs(raw: unknown): PublicIncidentRefV1[] {
  if (!Array.isArray(raw) || raw.length > MAX_INCIDENT_REFS) {
    throw new PublicEvidenceValidationError("invalid_incident_refs", "incidentRefs is invalid or oversized");
  }
  const seen = new Set<string>();
  return raw.map((value, index) => {
    if (!isPlainObject(value)) {
      throw new PublicEvidenceValidationError("invalid_incident_ref", `incidentRefs[${index}]`);
    }
    ...
    if (!isPublicIncidentRef(corpusId) || seen.has(corpusId)) {
      throw new PublicEvidenceValidationError("invalid_incident_ref", `incidentRefs[${index}].corpusId`);
    }
    seen.add(corpusId);
    return { corpusId };
  });
...
function validateProtocolSubject(raw: Record<string, unknown>): PublicProtocolSubjectV1 {
  assertClosedKeys(raw, [
    "subjectKind", "adapterFamily", "inboundProtocol", "upstreamProtocol", "surface", "compatibilityVersion",
  ], "subject");
  if (raw.subjectKind !== "protocol") {
    throw new PublicEvidenceValidationError("subject_kind", "protocol subjectKind mismatch");
  }
  return {
...
function validateRouteSubject(raw: Record<string, unknown>): PublicRouteSubjectV1 {
  assertClosedKeys(raw, [
    "subjectKind", "providerId", "modelId", "adapterFamily", "inboundProtocol", "upstreamProtocol", "surface",
    "compatibilityVersion", "registryVersion", "registryDigest",
  ], "subject");
  if (raw.subjectKind !== "route") {
    throw new PublicEvidenceValidationError("subject_kind", "route subjectKind mismatch");
  }
  return {
    subjectKind: "route",
...
function validateTaskSubject(raw: Record<string, unknown>): PublicTaskSubjectV1 {
  ...
  if (raw.subjectKind !== "task" || !isPlainObject(raw.route)) {
    throw new PublicEvidenceValidationError("subject_kind", "task subject is invalid");
  }
  return {
    subjectKind: "task",
...
function validateSubject(raw: unknown, layer: string): PublicEvidenceSubjectV1 {
  if (!isPlainObject(raw)) {
    throw new PublicEvidenceValidationError("invalid_subject", "subject must be object");
  }
...
export function validatePublicEvidenceRecord(raw: unknown): PublicEvidenceRecordV1 {
  ...
  assertClosedKeys(raw, [
    "recordId", "subjectId", "evidenceLayer", "suiteId", "suiteVersion", "scenarioId", "scenarioVersion", "verdict",
    "observedDayUtc", "subject", "assertions", "incidentRefs", "artifactRefs",
  ], "record");

  const layer = assertString(raw.evidenceLayer, "evidenceLayer", 64);
  if (!(EVIDENCE_LAYERS as readonly string[]).includes(layer)) {
    throw new PublicEvidenceValidationError("closed_set", "evidenceLayer");
  }
  const verdict = assertString(raw.verdict, "verdict", 32);
  if (!(VERDICTS as readonly string[]).includes(verdict)) {
    throw new PublicEvidenceValidationError("closed_set", "verdict");
  }
  if (!Array.isArray(raw.assertions) || raw.assertions.length > MAX_ASSERTIONS) {
    throw new PublicEvidenceValidationError("invalid_assertions", "assertions is invalid or oversized");
  }
  const assertions = raw.assertions.map((value, index) => {
    ...
    return {
      id: assertPublicToken(value.id, `assertions[${index}].id`, 256),
      required: assertBoolean(value.required, `assertions[${index}].required`),
      passed: assertBoolean(value.passed, `assertions[${index}].passed`),
    };
  });

  let artifactRefs: string[] | undefined;
  if (raw.artifactRefs !== undefined) {
    if (!Array.isArray(raw.artifactRefs) || raw.artifactRefs.length > MAX_ARTIFACT_REFS) {
      throw new PublicEvidenceValidationError("invalid_artifact_refs", "artifactRefs is invalid or oversized");
    }
    ...
    if (new Set(artifactRefs).size !== artifactRefs.length) {
      throw new PublicEvidenceValidationError("duplicate_artifact_ref", "artifactRefs contains duplicates");
    }
  }
  ...
  if (new TextEncoder().encode(jcsStringify(record)).byteLength > MAX_RECORD_BYTES) {
    throw new PublicEvidenceValidationError("record_too_large", `record exceeds ${MAX_RECORD_BYTES} bytes`);
  }
  return record;
}
...
export function validatePublicRouteRegistryManifest(raw: unknown): PublicRouteRegistryManifestV1 {
  ...
  const entries: PublicRouteRegistryEntryV1[] = raw.entries.map((value, index) => {
    ...
    assertClosedKeys(value, ["providerId", "modelId", "adapterFamilies"], `entries[${index}]`);
    ...
    if (!Array.isArray(value.adapterFamilies) || value.adapterFamilies.length === 0 || value.adapterFamilies.length > 3) {
      throw new PublicEvidenceValidationError("invalid_registry_entry", `entries[${index}].adapterFamilies`);
    }
    const adapterFamilies = value.adapterFamilies.map((adapter, adapterIndex) =>
      assertAdapter(adapter, `entries[${index}].adapterFamilies[${adapterIndex}]`));
    if (new Set(adapterFamilies).size !== adapterFamilies.length) {
      throw new PublicEvidenceValidationError("invalid_registry_entry", `entries[${index}].adapterFamilies duplicates`);
    }
    ...
    seen.add(key);
    return { providerId, modelId, adapterFamilies };
  });
  const manifestDigest = assertHex(raw.manifestDigest, "manifestDigest");
  const expected = publicRouteRegistryDigest({
  ...
  if (manifestDigest !== expected) {
    throw new PublicEvidenceValidationError("registry_digest_mismatch", "manifestDigest does not match canonical registry bytes");
  }
  return {
    schemaVersion: PUBLIC_ROUTE_REGISTRY_SCHEMA_VERSION,
    registryVersion,
