...
function validateRouteAuthority(subject: PublicRouteSubjectV1): void {
  if (subject.registryVersion !== PUBLIC_ROUTE_REGISTRY_V1.registryVersion || subject.registryDigest !== PUBLIC_ROUTE_REGISTRY_V1.manifestDigest) throw new PublicEvidenceValidationError("community_authority", "public route registry authority mismatch");
  if (!findPublicRouteRegistryEntry(subject.providerId, subject.modelId, subject.adapterFamily)) throw new PublicEvidenceValidationError("community_authority", "public route is not in reviewed registry authority");
}
function validateScenarioAuthority(record: PublicEvidenceRecordV1): void {
  if (record.evidenceLayer === "task_effectiveness") {
    if (record.suiteId !== FABRIC_SUITE_ID || record.suiteVersion !== FABRIC_SUITE_VERSION || record.scenarioId !== FABRIC_SCENARIO_ID || record.scenarioVersion !== FABRIC_SCENARIO_VERSION || record.subject.subjectKind !== "task" || record.subject.taskClassId !== FABRIC_SCENARIO_ID || record.subject.taskClassVersion !== FABRIC_SCENARIO_VERSION || record.subject.verifierAuthorityId !== FABRIC_VERIFIER_ID) throw new PublicEvidenceValidationError("community_authority", "task scenario/verifier authority mismatch");
  ...
  if (!caseRecord || caseRecord.suite !== record.suiteId || record.scenarioVersion !== String(authority.manifestDefaults.version) || record.suiteVersion !== String(authority.manifestDefaults.suiteVersion)) throw new PublicEvidenceValidationError("community_authority", "scenario/suite authority mismatch");
  if (record.evidenceLayer === "live_route_compatibility") {
    if (record.subject.subjectKind !== "route") throw new PublicEvidenceValidationError("community_authority", "live route subject mismatch");
