 ...
 * preparing. The toggle import is resolved eagerly only when the flag is
 * already on, so a clean-home v1 injection does not load the CLI module.
 */
export async function prepareInjectedV1SurfaceReconcile(
  ...
  return {
    ...
    run() {
      ...
      let active = toggle ?? toggleForTests;
      if (!active) {
        // The flag flipped on between prepare and the lock — reachable only on
        // the uncoordinated legacy path, which has no admission re-check.
        const { runCodexFeaturesCommand } = require("../../cli/v2") as typeof import("../../cli/v2");
        active = enabled => runCodexFeaturesCommand(enabled ? "enable" : "disable");
      }
      const transition = transitionMultiAgentV2(false, active);
      if (!transition.ok) {
        return {
          ok: false,
