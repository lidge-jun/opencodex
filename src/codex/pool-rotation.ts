...
export const POOL_KEY_COMMAND_CODE = "command-code";
...
export function reconcilePoolRotationState(context: GenerationContext): number {
  ...
  const commandCodeIds = new Set<string>();
  for (const key of context.oauthAccountKeys) {
    ...
    if (separator > 0 && key.slice(0, separator) === "command-code") {
      commandCodeIds.add(key.slice(separator + 1));
    }
  }
  let removed = 0;
