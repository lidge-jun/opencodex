...
выбранному маршрутизацией. Vision-сайдкар поддерживает OpenAI через вход ChatGPT, Anthropic через OAuth
или предназначенный только для vision Chat-бэкенд с явной целью `provider/model`; веб-поиск использует свои
отдельные бэкенды.
