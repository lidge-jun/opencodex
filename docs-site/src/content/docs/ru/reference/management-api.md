...
| `GET, POST /api/custom-models` | Показывать или добавлять custom-модели без `codexAccountTarget` и `codexAccountTargetWriteNonce`. Legacy POST-запись с target или nonce отклоняется с 409 (используйте `/api/custom-models/account-target`). | 400 invalid fields/provider; 404 provider missing; 409 duplicate/ambiguous model ID or legacy target write |
| `POST /api/custom-models/account-target` | Добавлять custom-модель со строковым `codexAccountTarget` и временным UUID `codexAccountTargetWriteNonce`. Цель допустима только для canonical `openai` Codex-forward и должна указывать на существующий Pool-аккаунт. Nonce только возвращается эхом и не сохраняется. | 400 invalid/missing fields/target/nonce/provider; 404 provider missing; 409 duplicate/ambiguous model ID or config conflict |
| `PUT, DELETE /api/custom-models/{id}` | Изменять или удалять custom-модель без target-полей. Legacy PUT отклоняет target-поля и изменения строки с уже существующей или некорректной целью (используйте `/api/custom-models/{id}/account-target`). Один nonce для непривязанной строки игнорируется ради совместимости со старыми клиентами. | 400 invalid id/fields; 404 not found; 409 duplicate/ambiguous model ID or legacy target write |
| `PUT /api/custom-models/{id}/account-target` | Задать или снять точную привязку. Требуются временный UUID `codexAccountTargetWriteNonce` и либо `codexAccountTarget` (`null` снимает привязку), либо существующее свойство цели строки для неизменной коррекции. Новый неизвестный Pool-ID отклоняется. Nonce только возвращается эхом и не сохраняется. | 400 invalid id/target/nonce; 404 not found; 409 duplicate/ambiguous model ID or config conflict |
| `GET /api/codex-auth/account-target-options` | Вернуть безопасные метаданные выбора аккаунта без чтения credentials или quota | — |
