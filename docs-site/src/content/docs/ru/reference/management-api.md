...
| Метод и путь | Назначение | Особые ошибки |
| --- | --- | --- |
| `GET /api/catalog` | Вернуть установленный документ каталога Codex | 404 catalog not found; 500 каталог содержит значения, которые нельзя распространять; 500 файл каталога слишком велик для чтения |
...
Оба маршрута используют один и тот же шаг материализации, поэтому этот маршрут наследует два его
отказа. Каждый отвечается в обычном management-envelope — `{ "error": "<message>" }` — а не в
envelope data plane с `type`/`code`, поэтому коды ошибок `/v1/catalog` здесь не появляются:

| Статус | Тело ответа | Значение |
| --- | --- | --- |
| 404 | `{ "error": "catalog not found" }` | Не удалось прочитать ни один документ каталога |
| 500 | `{ "error": "catalog contains values that are not safe to distribute" }` | Сохранённый каталог несёт содержимое, похожее на credential, идентичность или конфигурацию. Сообщение намеренно не содержит содержимого |
| 500 | `{ "error": "catalog file is too large to read" }` | Сохранённый файл превышает безопасный предел чтения источника в 32 MiB и был отклонён до разбора |

Этот маршрут **не** применяет предел сериализованного ответа data plane в 8 MiB; эта граница
принадлежит только удалённо достижимому транспорту.

:::note
Применение общего отказа к management-маршруту — следствие единственного шага материализации, а не
одобренная политика: раньше этот маршрут отдавал любой разбираемый файл каталога. Должен ли
management plane оставаться под тем же отказом — открытое решение мейнтейнеров, зафиксированное в
