...
| `POST /api/codex-auth/reset-credits/consume` | Израсходовать reset credit; требуется GUI session с повторно введённым owner admin token или одноразовый локальный consent capability CLI. Одной admin auth или поля `confirmed` недостаточно. Вызывающая сторона должна передать стабильный канонический UUIDv4 в нижнем регистре как `operationId` и надёжно повторно использовать его до получения ответа с terminal code; quota refresh выполняется отдельным последующим чтением | 400 отсутствующий/недопустимый `accountId` или `operationId`; 403 `agent_consent_required`; 409 `reset_credit_operation_identity_changed`; upstream status passthrough; 503 `server_busy`; 507 `reset_credit_operation_history_full` (требуется действие сопровождающего, без автоматического повтора); 500 consume failure |
