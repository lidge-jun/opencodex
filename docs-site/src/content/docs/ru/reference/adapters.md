...
- Для `cursor/grok-4.5` предоставляет уровни reasoning до `high`, а для `cursor/grok-4.6` — также
  `xhigh`. Обе соответствующие строки `-fast` остаются доступными для выбора; Cursor получает
  каноническую базовую модель Grok, а отдельные значения `effort` и `fast=true` помещаются в
