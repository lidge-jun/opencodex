...
**Назначение:** публичный beta-веб-клиент Upstage Open2 с WebSocket-фреймами `solar-chat.v1`.
**Аутентификация:** анонимная cookie `solar_session`, автоматически создаваемая и обновляемая через
`GET /api/session`; API-ключ сейчас не требуется. Настроенный API-ключ игнорируется и никогда не
используется как HTTP- или WebSocket-cookie.

- Это неофициальный bridge к private-beta веб-протоколу, а не стабильный публичный API. Он бесплатен
  и не требует ключа только пока открыта анонимная публичная beta; Upstage может без предупреждения
  изменить, ограничить, потребовать аутентификацию или удалить протокол.
- Использует `runTurn` для жизненного цикла WebSocket и проверяет ready protocol и последовательность
  событий. Отдельные накопительные snapshot'ы usage сохраняются и объединяются с финальным usage без
  двойного подсчёта.
- Обновлённые session-cookie хранятся только в памяти процесса, не записываются на диск и теряются
  после перезапуска OpenCodex.
- `none` — sentinel отключения reasoning. Каталог объявляет `medium`, `high` и `max`; `low`/`minimal`
  отображаются в `medium`, а `xhigh` — в `high`.
- Hosted web search не объявляется, поскольку `runTurn` обходит sidecar. Текущий upstream wire также
  не поддерживает client tools Codex и native vision.
- При настроенном исходящем HTTP(S)-proxy адаптер завершается с ошибкой, чтобы WebSocket не обошёл
  proxy. Добавляйте хост Open2 в `NO_PROXY` только если прямое подключение действительно ожидается.
