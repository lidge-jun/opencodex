...
2. `server/responses/core.ts` распаковывает и парсит JSON, разворачивает локально запомненный вход
   `previous_response_id`, когда он доступен (полная переотправка истории сохраняется без повторного
   добавления сохранённой истории), затем вызывает `responses/parser.ts`; вход, оценка которого
   превышает действующий лимит ввода маршрутизируемой модели, отклоняется с `413 request_too_large`
   до любого upstream-I/O.
