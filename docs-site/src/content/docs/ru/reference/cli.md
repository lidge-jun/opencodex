...
Для здорового прокси `version` и `uptimeSeconds` содержат структурированные значения из `/healthz`, если эндпоинт их предоставляет; любое поле может быть `null`, если прокси его опускает.
