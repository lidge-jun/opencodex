...
| `proxy?` | `string` | — | URL исходящего HTTP(S) или SOCKS-прокси (`socks5://host:port`) или `${ENV_VAR}`. HTTP URL пишутся в `HTTP_PROXY` / `HTTPS_PROXY`, если те не заданы. SOCKS URL пишутся в `ALL_PROXY` (`ocx start --socks5`) и сбрасывают унаследованные `HTTP(S)_PROXY` в этом процессе. Loopback всегда остаётся в `NO_PROXY`. |
