...
Явный серверный прокси `socks5://` или `socks5h://` использует встроенный туннель OpenCodex;
унаследованный SOCKS5 `ALL_PROXY` использует его же, если цель не совпадает с `NO_PROXY`.
