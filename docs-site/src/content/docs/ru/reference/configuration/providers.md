...
reauth или порога исчерпания; здоровые привязанные задачи сохраняют affinity. `reset-window`
упорядочивает подходящие аккаунты по свежему будущему времени governing reset в направлении
`accountPoolResetOrder`, пропускает аккаунт выше порога, пока у другого есть запас, и возвращается
к `quota`, если данные отсутствуют или устарели. Эти стратегии не защищают от enforcement провайдера.
...
| `anthropicAccountPool.strategy?` | `"quota" \| "round-robin" \| "fill-first" \| "reset-window"` | `"quota"` | Стратегия для новых сессий; `quota` ранжирует аккаунты по окну `quotaWindow`, `fill-first` в этом же окне оценивает порог исчерпания, а `reset-window` использует свежий будущий недельный сброс и возвращается к quota, если эти данные отсутствуют или устарели. |
...
| `anthropicAccountPool.resetOrder?` | `"soonest" \| "latest"` | `"soonest"` | Сначала выбирает ближайший или самый далёкий недельный сброс. |
