...
запросе после превышения порога, и только на подходящий аккаунт, у которого ещё есть запас ниже порога; при включённом `pool.cacheAffinity` эта перепривязка ждёт, пока
