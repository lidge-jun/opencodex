...
### `googleAntigravityAccountPool` (экспериментальная функция)

Эта opt-in функция объединяет Google Antigravity OAuth-аккаунты только для Cloud Code Assist
...
| Ключ | Тип | По умолчанию | Описание |
| --- | --- | --- | --- |
| `googleAntigravityAccountPool.enabled?` | `boolean` | `false` | Включить локальную для процесса привязку сессий и ограниченный failover после финального 429 или дошедшего до прокси 402. |
| `googleAntigravityAccountPool.autoSwitchThreshold?` | `number` | `80` | Порог расходования от 0 до 100. `quota` и `fill-first` берут максимальное cached usage, относящееся к семейству `Gem` или `Cla` запрошенной модели; при неизвестном usage сохраняется здоровый активный аккаунт. `0` отключает переключение по usage, но не восстановление после сбоев. |
| `googleAntigravityAccountPool.strategy?` | `"quota" \| "round-robin" \| "fill-first"` | `"quota"` | Стратегия для новых или ещё не привязанных сессий. |
| `googleAntigravityAccountPool.stickyLimit?` | `number` | `1` | Число привязок новых сессий, удерживаемых на одном выборе `round-robin`. Диапазон 1–100; на другие стратегии не влияет. |

Все стратегии сохраняют существующую привязку сессии, пока аккаунт остаётся подходящим. Для новой
или непривязанной сессии `quota` сохраняет здоровый активный аккаунт, пока относящееся к модели usage
неизвестно или ниже порога, а затем выбирает подходящий аккаунт с наименьшим известным usage.
`round-robin` равномерно распределяет привязки и не использует порог при обычной ротации. `fill-first`
заполняет активный аккаунт до cooldown, недоступности или порога расходования, затем переходит к
следующему подходящему аккаунту.

Для каждого dispatch разрешается один привязанный к поколению OAuth snapshot, содержащий bearer token
и Cloud Code Assist `projectId`. Финальный 429 или дошедший до прокси 402 переводит сбойный аккаунт в
cooldown, очищает его привязку и пересобирает запрос для следующего подходящего snapshot. Если пригодного
`Retry-After` нет, cooldown по умолчанию равен 60 секундам; распознанное значение upstream ограничено
15 минутами. Один запрос допускает не более трёх failover, то есть всего четырёх upstream dispatch.
Если все подходящие аккаунты в cooldown, клиент получает 429 с самым ранним известным `Retry-After`.

Специализированная ротация при 402/429 ограничена обычным основным dispatch-путём Responses и terminal
continuation. При маршрутизации через Google циклы `image/video bridge` и `web-search sidecar` могут
использовать аккаунт, изначально выбранный для запроса, но не переводят его в cooldown и не ротируют
через этот специализированный пул. Отдельный endpoint изображений Antigravity также находится вне
этого пути ротации.

:::caution[Операционная устойчивость, а не обход квот]
Используйте пул для восстановления после временных сбоев аккаунта, а не для обхода квот или enforcement
провайдера. Автоматизация нескольких аккаунтов может нарушать условия провайдера; оставьте функцию
выключенной, если вы не принимаете этот риск.
:::
