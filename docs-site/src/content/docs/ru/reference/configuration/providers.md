...
| `anthropicAccountPool.strategy?` | `"quota" \| "round-robin" \| "fill-first" \| "reset-window"` | `"quota"` | Стратегия для новых сессий; `quota` ранжирует аккаунты по окну `quotaWindow`, `fill-first` в этом же окне оценивает порог исчерпания, а `reset-window` использует свежий недельный сброс. |
