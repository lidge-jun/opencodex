...
Discovery использует настроенный API-ключ OpenRouter. Отказ авторизации не меняет сохранённую маршрутизацию. Успешный результат кэшируется на пять минут; **Обновить** обходит кэш даже после пустого результата.
