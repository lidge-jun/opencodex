...
Экспериментальное восстановление по умолчанию выключено. Включайте его через **Подагенты → Настройки → Восстанавливать зашифрованные v2-задачи** только после подтверждения предупреждения о quota и latency либо используйте `ocx agent recovery status|on|off`. Эквивалентный API — `GET/PUT /api/agent-task-recovery`; `PUT` принимает только `{"enabled": boolean}`, сначала сохраняет durable-конфигурацию, затем обновляет активное состояние и сохраняет расширенные параметры, заданные вручную.
