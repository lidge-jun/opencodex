...
| `strategy?` | `"failover" \| "round-robin" \| "random" \| "least-used" \| "reset-window"` | `"failover"` | Стратегия выбора. Порядок целей разрешает ничьи; веса используются в round-robin и random. |
| `stickyLimit?` | `number` | `1` | Число успешных запросов в одной партии round-robin. Диапазон 1–100; остальные стратегии игнорируют это поле. |
