...
Если `cooldownMs` не задан, цель после hop использует upstream fallback: 5 секунд для 429, ограничивающих частоту запросов, с кодом upstream `1302` или `1305`, 10 минут для исчерпанного окна квоты аккаунта и 60 секунд в остальных случаях. Исчерпанное окно квоты определяется по коду upstream (`usage_limit_exceeded`, `usage_limit_reached`, `1308`) или по тексту `usage limit reached`, независимо от HTTP-статуса: бэкенд ChatGPT Codex сообщает об исчерпанном окне ответом 502, а не документированным 429. Если он задан, `cooldownMs` применяется, когда нет пригодного сигнала upstream `Retry-After` или сигнала сброса Codex, включая такие 429, ограничивающие частоту запросов. Принимаются числовые секунды в `Retry-After` и значения HTTP-date; явный `Retry-After` ограничен 24 часами, а cooldown по сбросу, настройке или fallback — 10 минутами. Приоритет от сильного к слабому: явный `Retry-After` → заголовки сброса Codex (`x-codex-primary-reset-at`, `x-codex-secondary-reset-at` или `x-codex-tertiary-reset-at`) → `cooldownMs` этой combo (если задан) → 10-минутный cooldown для исчерпанного окна квоты → 5-секундный fallback для rate-limit-кодов upstream `1302`/`1305` → стандартные 60 секунд. Окно квоты проверяется первым, поэтому сбой, несущий и rate-limit-код, и текст об исчерпании лимита, удерживается 10 минут, а не 5 секунд. Корректный немедленный `Retry-After: 0` сохраняется как немедленная директива upstream, а не заменяется настроенным cooldown.
...
| `cooldownMs` | No | не задано → fallback upstream (5 с для rate-limit 429 с кодами `1302`/`1305`, 10 мин для исчерпанного окна квоты, иначе 60 с) | Целое число от 1 до 600000. Если задано, применяется как cooldown каждой цели, когда нет пригодного upstream `Retry-After` или сигнала сброса Codex, включая rate-limit 429; если не задано, используется fallback upstream. |
...
провайдеров цели и недавние upstream-ошибки. Для cooldown сначала следуйте значению `Retry-After` из ответа. Заголовки сброса Codex также имеют приоритет над `cooldownMs`, поэтому если ни один upstream-сигнал не пригоден, применяется заданный `cooldownMs`, а если он не задан — upstream fallback (5 секунд для rate-limit-кодов `1302`/`1305`, 10 минут для исчерпанного окна квоты, иначе 60 секунд); явный `Retry-After` ограничен 24 часами, а cooldown по сбросу, настройке или fallback — 10 минутами.
