...
`PATH`. При запуске и выполнении `ocx ensure` хук, принадлежащий OpenCodex, удаляется, если Claude Code
не установлен или системная интеграция окружения выключена. Claude Desktop использует отдельный профиль
и не вызывает установку shell-хука.
