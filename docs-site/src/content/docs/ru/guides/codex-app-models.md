...
недоступности цели завершаются ошибкой без переключения аккаунта. Имена селекторов — настраиваемые публичные
метки. При инициализации создаются безопасные для конфиденциальности значения по умолчанию; встроенного смысла
роли аккаунта у них нет. Выбор строки с селектором использует
...
`<selector>` ниже — настраиваемая публичная метка, сопоставленная через `codexAccountNamespaces`;
для инициализированной привязки значение может быть создано автоматически:
