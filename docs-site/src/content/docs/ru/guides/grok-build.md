...
закреплённые upstream-шкалы. Уровни только для Codex, например `ultra`, исключаются из файла,
поскольку стандартная шкала Grok CLI заканчивается на `max`; каждый записанный пункт остаётся
