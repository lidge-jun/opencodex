...
- **`ocx restart`, установленный как service:** когда opencodex работает под service manager,
  `ocx restart` сейчас останавливает службу и заменяет её неуправляемым процессом — persistence
  службы (автоперезапуск, старт при логине) теряется до следующего `ocx service`, а если этот
  неуправляемый процесс погибнет, managed block может указывать на уже мёртвый прокси, пока
  следующий `ocx start`/`ocx ensure` не обновит его.
