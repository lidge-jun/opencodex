...
- Каждое изображение отправляется в настроенную нативную vision-модель с выбранным для OpenAI
  Responses значением `reasoning.effort` (по умолчанию `low`); полученное описание заменяет часть
  с изображением на месте. Anthropic vision игнорирует эту специфичную для OpenAI настройку.
