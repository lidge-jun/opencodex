...
Квота Antigravity запрашивается вживую через RPC CCA `retrieveUserQuota` и `retrieveUserQuotaSummary` (тайм-аут 8 секунд); при недоступности живой квоты используется поддерживаемый каталог.
