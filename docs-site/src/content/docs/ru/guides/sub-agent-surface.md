...
ChatGPT, выбор ограничивается каноническими нативными целями ChatGPT или Responses-провайдерами с
явным `allowEncryptedV2AgentTasks: true`.

## Доставка шифрованных задач v2

Codex может отправить задачу child v2 из native-to-routed пути только как backend-encrypted
`encrypted_content`. Нативный backend ChatGPT может её обработать, и некоторые совместимые relay могут передать её подходящему backend; OpenCodex не выводит эту способность из имени или Base URL. Это известное ограничение [#92](https://github.com/lidge-jun/opencodex/issues/92).

opencodex завершаетcя безопасно и не пересылает пустую или нечитаемую задачу:

- Прямой не-нативный маршрут без явного доверия возвращает HTTP 400 с
  `error.code = "unreadable_encrypted_agent_task"` и не отражает ciphertext назад.
- Combo для такой задачи рассматривает только канонические нативные цели ChatGPT и явно доверенные Responses-цели, включая retry.
...
После проверки неканонического Responses endpoint включите **Передавать зашифрованные задачи агентов V2** в настройках провайдера либо задайте `allowEncryptedV2AgentTasks: true`. Опция по умолчанию выключена, требует `adapter: "openai-responses"`, передаёт opaque-задачу без изменений и не доказывает совместимость или способность расшифровывать. Для такого маршрута `agentTaskRecovery` пропускается.
