...
При отмене повторной аутентификации основного аккаунта по коду устройства ответ HTTP 404 с кодом `unknown_flow` освобождает истёкший процесс, позволяя начать вход по коду устройства заново, но не сообщает об успешном входе или подтверждённой отмене. После подтверждённой отмены можно снова начать вход по коду устройства. Запоздалые ответы на запуск, проверку состояния или отмену прежнего процесса не изменяют новый процесс и не сообщают об успешном входе.
