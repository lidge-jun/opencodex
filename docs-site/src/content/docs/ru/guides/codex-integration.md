...
Настройка убирает только отдельный вход в Codex Desktop: она не создаёт и не выбирает учётные данные и не предоставляет доступ к аккаунтам. Pool выбирает настроенные аккаунты Codex; Direct по-прежнему требует учётные данные вызывающей стороны/основного аккаунта. Luna Reserve также требует действующего локального режима `codexDesktopAuthless`, учётных данных и актуального разрешения провайдера, связанного с этими данными. Наличие модели в каталоге само по себе не разрешает запрос. Luna Reserve поддерживается только через канонический адаптер ChatGPT-forward с разрешением вышестоящего сервиса, привязанным к учётным данным. Аутентификация по API-ключу и произвольные шлюзы Responses не подходят.
