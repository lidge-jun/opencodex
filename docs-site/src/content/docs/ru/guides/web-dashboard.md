...
Каждый переключатель на странице **Models** управляет точным идентификатором модели у всех провайдеров, которые её предлагают, включая добавленных позже: выключение скрывает этот идентификатор везде. Скрытие у отдельного провайдера показывается на строке отдельной меткой. Видимость для конкретного провайдера настраивается в **Providers → Models**: **Все выкл.** скрывает все модели провайдера, а **Все вкл.** возвращает их и удаляет allowlist, чтобы новые модели тоже включались. Любое скрытие имеет приоритет; восстановление на одном уровне не отменяет другое.
