...
| **Models** | Toggle native GPT and routed models, set provider allowlists and context caps, choose v1/base/v2, and configure the v2 thread limit. Configured providers stay visible as zero-model groups when discovery is off or returns no rows. When discovery fails, the badge hover keeps configured fallback models explicit and directs OAuth providers to re-login; other providers link to endpoint and discovery settings. |
