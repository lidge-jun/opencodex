...
| **Usage / Debug** | Inspect token-usage coverage and trends. The Usage page's Models table also breaks each model down into input tokens, output tokens, cache hits, cache writes, and cache hit rate; a dash means the provider supplied no cache telemetry. Or enable opt-in provider transport and usage-extraction diagnostics. |
