...
| **Logs** | Browse cursor-paginated persisted request metadata, with server-side provider/model/status/protocol/key/profile/fallback filters and a separate Debug tab. Details load the recorded route decision, attempt sequence, final target, outcome, tokens, duration, and list-price estimate. Pre-trace rows stay unknown rather than being reconstructed from the final provider. |
...
| `GET /api/request-history?limit=200&cursor=...` · `GET /api/request-history/:requestId/route-decision` | Read cursor-paginated persisted metadata and the recorded why-this-route evidence. The legacy `/api/logs` ring remains available for compatibility, but the dashboard history uses these endpoints. |
