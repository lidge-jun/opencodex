...
During validated Ark Coding Plan tool continuations, replaying the returned Responses `reasoning` item caused `400 InvalidParameter`, so the Coding Plan preset drops replayed reasoning items before forwarding continuation input. Explicit per-model `openai-chat` overrides remain available. The built-in DeepSeek preset also routes `deepseek-v4-flash` over its native Responses endpoint and
