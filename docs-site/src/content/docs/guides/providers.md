...
- **Model Discovery:** `codebuddy --help` prints the account-scoped roster (the "Currently supported" list), which is the authoritative model list: the same CLI build prints different models for different subscriptions, and it can differ from the static manifest bundled with the CLI. On start/sync the proxy reads that roster with the region's key, binds the cached result to an irreversible fingerprint of the key (an account switch never reuses another account's roster), and degrades to the stale account-scoped cache, then to the static seed in `src/providers/codebuddy-models.ts`, when the CLI is missing or the roster line cannot be parsed.
