...
- **Model Discovery:** `codebuddy --help` prints the account-scoped roster (the "Currently supported" list), which is the authoritative roster of discovered models: the same CLI build prints different models for different subscriptions, and it can differ from the static manifest bundled with the CLI. The vendor default selectors (`default` for CN, `default-model` for Global) never appear in that roster line but remain callable: the catalog retains them during live discovery and on every fallback path. The roster reflects the CodeBuddy account signed in under the caller's home and can differ from the key configured in OpenCodex: a probe with a deliberately wrong key still prints the signed-in account's roster. On start/sync the proxy binds the cached roster to an irreversible fingerprint of the configured key, so a key switch never observes a roster cached for the previous key, and degrades to the stale provider/key-fingerprint-scoped cache, then to the static seed in `src/providers/codebuddy-models.ts`, when the CLI is missing or the roster line cannot be parsed.
...
- **Entitlements and Billing:** The provider uses the same vendor-documented CodeBuddy account/CLI authentication surface. Availability and billing of free, promotional, trial, or subscription credits remain determined by the user's CodeBuddy account entitlement.### Official Qoder CLI (Global & CN)
