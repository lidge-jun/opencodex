...
Antigravity quota is probed live via CCA's `retrieveUserQuota` and `retrieveUserQuotaSummary` RPCs (8-second timeout); the maintained catalog remains the fallback when live quota is unavailable.
