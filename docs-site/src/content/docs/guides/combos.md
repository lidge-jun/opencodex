...
429s with upstream code `1302` or `1305`, 10 minutes for exhausted usage limits (including HTTP 502), and 60 seconds otherwise. When it is set, `cooldownMs`
...
`x-codex-tertiary-reset-at`) → the combo's `cooldownMs` (when set) → the 10-minute exhausted-usage-limit fallback → the 5-second request-rate
...
| `cooldownMs` | No | unset → upstream fallback (5 s for request-rate 429 codes `1302`/`1305`, 10 min for exhausted usage limits including HTTP 502, otherwise 60 s) | Integer from 1 to 600000. When set, applies as the per-target cooldown whenever no usable upstream `Retry-After` or Codex reset signal exists, including request-rate 429s; when unset, uses the upstream fallback. |
...
when it is unset (5 seconds for request-rate codes `1302`/`1305`, 10 minutes for exhausted usage limits including HTTP 502, otherwise 60 seconds). Explicit
