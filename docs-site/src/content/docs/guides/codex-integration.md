...
When an affected history store supports paginated records, a provider transition may return `history_paginated_requires_native_writer`. That reason no longer refuses the Codex configuration, the reference profile, or the model catalog. `ocx sync` and `ocx start` still write those files and set `model_catalog_json`, so the Codex model picker keeps showing every OpenCodex-routed model. Only the conversation-history relabel stands down, because paginated rollout ordinals may be allocated exclusively by Codex's own writer. OpenCodex never modifies paginated rollout files or thread rows in this state. Existing conversations keep whatever provider they are already tagged with and are not migrated; new conversations route through the proxy normally. This includes legacy rows in a migration-capable store. The CLI prints `Codex resume history: left to Codex's native writer (history_paginated_requires_native_writer)`.

`ocx restore` and Codex config removal are also no longer blocked by this reason. After restoring the journal or removing owned configuration, OpenCodex reverts the configuration and catalog and leaves conversation history to Codex's native writer.
