...
and `ocx-claude-<provider>--<model>` ids from older configs still resolve. A saved `claude-ocx-` id still routes, but Claude Code keeps its 200k accounting for that id. Pick the new `ocx-claude-` id once so the real context window and compact both apply.
