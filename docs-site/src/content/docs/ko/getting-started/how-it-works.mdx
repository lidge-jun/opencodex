...
4. **Vision sidecar(선택)** — 라우팅된 모델이 `provider.noVisionModels`에 나열되어 있고
   요청에 이미지가 포함된 경우, opencodex는 설정된 vision backend(ChatGPT 로그인 기반 OpenAI,
   OAuth 기반 Anthropic, 또는 명시적 `provider/model`을 사용하는 vision 전용 Chat backend)로 각 이미지를
   설명한 뒤 텍스트로 치환합니다. 덕분에 텍스트 전용 모델도 해당 이미지에 대해 추론할 수 있습니다.
