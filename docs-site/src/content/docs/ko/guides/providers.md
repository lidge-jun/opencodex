...
Google 도구 선언은 선택된 엔드포인트 클래스에 맞춰 컴파일됩니다. 프로바이더 디버그를
활성화하면 호환성을 위해 제약을 완화한 결과로 엔드포인트 클래스, 고정된 손실 범주,
상한이 있는 개수, 잘림 여부만 기록합니다. 도구명과 속성명, 경로, 값, 스키마 본문은
포함하지 않습니다. 이 진단은 기존 호환 변환을 관찰할 뿐 요청을 거부하지 않습니다.
네이티브 출력 스키마는 이 진단의 대상이 아닙니다.
