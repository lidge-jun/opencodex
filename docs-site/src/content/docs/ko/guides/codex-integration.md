...
메인 계정의 기기 코드 재인증을 취소할 때 HTTP 404와 `unknown_flow` 코드가 반환되면 만료된 흐름을 해제하여 기기 코드 재로그인을 다시 시작할 수 있게 하며, 로그인 성공이나 취소 확정으로 표시하지 않습니다. 취소가 확인된 뒤에도 기기 코드 재로그인을 다시 시작할 수 있습니다. 이전 흐름의 시작·상태 조회·취소 응답이 늦게 도착해도 새 흐름을 변경하거나 로그인 성공을 알리지 않습니다.
