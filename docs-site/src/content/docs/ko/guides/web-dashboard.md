...
**Models** 페이지의 각 스위치는 정확히 일치하는 모델 ID를 그 모델을 제공하는 모든 프로바이더(나중에 추가한 프로바이더 포함)에서 함께 제어합니다. 끄면 해당 ID가 모든 곳에서 숨겨집니다. 프로바이더별 숨김은 행에 별도 칩으로 표시됩니다. 프로바이더별 노출은 **Providers → Models**에서 설정하며, **모두 끄기**는 그 프로바이더의 모든 모델을 숨기고 **모두 켜기**는 다시 표시하면서 allowlist를 해제해 새로 발견되는 모델도 켭니다. 어느 쪽 숨김이든 우선하며, 한쪽을 되돌려도 다른 쪽은 해제되지 않습니다.
