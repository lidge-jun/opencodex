...
모든 페이지 상단의 한 줄 요약에 공급자별 현재 quota 사용률이 표시됩니다. 예: `OpenAI 31% | Claude 54% | xAI 12% | Google 8%`. 공급자 작업 화면과 같은 quota 보고(`GET /api/provider-quotas`)를 탭이 보이는 동안 60초마다 읽으며, 업스트림 강제 새로고침은 하지 않습니다.

- 각 항목은 보고된 가장 긴 창을 표시합니다. 주간, 30일, 5시간, 공급자 고유 창 또는 선불 크레딧 순서입니다.
- 70% 이상 사용하면 주황색, 90% 이상이면 빨간색으로 표시됩니다.
- 항목에 마우스를 올리거나 클릭하면 보고된 모든 창의 사용률, 초기화 시각, 데이터 기준 시각이 나옵니다. 고정된 항목은 Escape 키나 바깥 클릭으로 닫습니다.
- quota 창을 보고하지 않는 공급자는 표시하지 않습니다. 보고하는 공급자가 없으면 요약 바 전체가 숨겨집니다.
