...
| `requestPacing?` | `{ enabled, requestsPerMinute?, minIntervalMs?, maxConcurrentRequests?, models? }` | 업스트림 사용량, 과금, rate-limit 지표와 별개인 선택적 클라이언트 측 아웃바운드 요청 조절입니다. `maxConcurrentRequests`는 이 프로바이더로 동시에 진행 중인 요청 수의 상한이며, 업스트림 응답 본문이 닫힐 때까지 세습니다. 읽거나 취소하지 않은 본문은 30초 후, 부분적으로 읽힌 뒤 멈춘 본문은 300초 비활동 후 리스를 반환합니다.`runTurn` 어댑터는 물리적 전송이 아니라 턴마다 하나의 리스를 가지며, 유지 중인 턴 내 후속 전송(예: Cursor 재다이얼)은 간격만으로 허용되고 자기 턴의 리스 뒤에 대기하지 않습니다. Provider 제한은 모든 모델에 적용되고 `models` 항목은 정확한 업스트림 모델 ID와 일치하며 지연을 더 늘리거나 동시 상한을 더 좁힐 때만 적용됩니다. 큐 대기는 응답 헤더 타임아웃을 소모하지 않습니다. HTTP, 명시적 어댑터 `fetchResponse` 전송과 `runTurn` 시도를 포함합니다. 동시 상한을 설정하면 리스를 반환할 수 있도록 Responses WebSocket은 HTTP/SSE로 전환됩니다. |
