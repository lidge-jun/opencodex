...
특수 402/429 장애 순환은 일반 Responses 주 전송 경로와 터미널 연속 경로로 제한됩니다. Google로
라우팅될 때 `image/video bridge`와 `web-search sidecar` 루프는 요청에서 처음 선택한 계정을 사용할 수
있지만, 이 특수 풀을 통해 해당 계정을 쿨다운하거나 순환하지 않습니다. 독립형 Antigravity 이미지
엔드포인트도 이 순환 경로의 범위 밖입니다.
