...
| `subagentModels?` | `string[]` | `gpt-5.5`, `gpt-5.6-sol`, `gpt-5.6-terra`, `gpt-5.6-luna`, `gpt-5.4-mini` | 최대 다섯 개의 네이티브 또는 라우팅된 id를 서브에이전트 선택기에서 우선 표시합니다. 명시적으로 빈 목록을 넣어도 그대로 보존합니다. |
