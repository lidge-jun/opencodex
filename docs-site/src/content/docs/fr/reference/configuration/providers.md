...
| `supportsServiceTier?` | `boolean` | Capacité `service_tier` à trois états. `true` : le mode rapide peut injecter et les valeurs de l'appelant sont conservées. `false` : le champ est dépouillé et jamais injecté (l'amont documenté comme ne le supportant pas ne doit pas le recevoir). Absent : le fournisseur n'est pas classé : les valeurs fournies par l'appelant sont préservées intactes et le mode rapide n'effectue jamais d'injection. Le registre classe les canoniques OpenAI (`true`), DeepSeek et Volcengine Ark (`false`) ; définissez-le explicitement uniquement pour les passerelles personnalisées qui prennent réellement en charge les niveaux. |
...
| `parallelToolCalls?` | `boolean` | Activer/désactiver les appels d'outils parallèles. OpenAI Le chat est activé par défaut ; les adaptateurs non-chat font de la publicité uniquement sur `true` explicite. |
