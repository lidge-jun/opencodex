...
| `reset-window` | Classe les comptes admissibles selon leur réinitialisation directrice future et encore valide. `accountPoolResetOrder` choisit la plus proche (`soonest`) ou la plus éloignée (`latest`). Un compte au-dessus de `autoSwitchThreshold` est ignoré lorsqu'un autre dispose de marge ; des données absentes ou périmées reviennent à `quota`. Les tâches liées et saines conservent leur affinité. |
...
| `anthropicAccountPool.strategy?` | `"quota" \| "round-robin" \| "fill-first" \| "reset-window"` | `"quota"` | Stratégie des nouvelles sessions ; `quota` classe les comptes selon la fenêtre définie par `quotaWindow`, `fill-first` évalue son seuil d'évacuation dans cette même fenêtre et `reset-window` utilise une réinitialisation hebdomadaire future et encore valide, puis revient à quota si cette donnée est absente ou périmée. |
...
| `anthropicAccountPool.resetOrder?` | `"soonest" \| "latest"` | `"soonest"` | Choisit d'abord la réinitialisation hebdomadaire la plus proche ou la plus éloignée. |
