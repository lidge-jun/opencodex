...
| `proxy?` | `string` | — | URL du proxy HTTP(S) ou SOCKS sortant (`socks5://host:port`) ou `${ENV_VAR}`. Les URL HTTP s’appliquent à `HTTP_PROXY` / `HTTPS_PROXY` si elles sont vides. Les URL SOCKS s’appliquent à `ALL_PROXY` (`ocx start --socks5`) et effacent `HTTP(S)_PROXY` héritées dans ce processus. Le bouclage reste dans `NO_PROXY`. |
