...
| `GET, POST /api/custom-models` | Répertorier les modèles personnalisés ou en ajouter un sans `codexAccountTarget` ni `codexAccountTargetWriteNonce`. Les écritures POST héritées avec une cible ou un nonce sont rejetées ; utilisez `/api/custom-models/account-target`. | 400 champs/fournisseur invalides ; 404 fournisseur manquant ; 409 modèle en double/ambigu ou écriture de cible héritée |
| `POST /api/custom-models/account-target` | Ajouter une ligne personnalisée canonique `openai` avec transfert Codex, une chaîne `codexAccountTarget` et un UUID temporaire `codexAccountTargetWriteNonce`. La cible doit être `@main` ou l'identifiant d'un compte Pool actuellement présent. Le nonce est renvoyé uniquement en écho et n'est jamais persisté. | 400 champs/cible/nonce/fournisseur invalides ; 404 fournisseur manquant ; 409 modèle en double/ambigu ou conflit de configuration |
| `PUT, DELETE /api/custom-models/{id}` | Modifier ou supprimer un modèle personnalisé sans champs de cible. Le PUT hérité rejette les champs de cible et les modifications d'une ligne qui contient déjà une cible ou une cible mal formée ; utilisez `/api/custom-models/{id}/account-target`. Un nonce seul est ignoré pour la compatibilité avec les anciens clients sur une ligne non liée. | 400 id/champs invalides ; 404 introuvable ; 409 modèle en double/ambigu ou écriture de cible héritée |
| `PUT /api/custom-models/{id}/account-target` | Définir ou effacer la liaison exacte. Requiert un UUID temporaire `codexAccountTargetWriteNonce` ainsi que `codexAccountTarget` (`null` efface) ou la propriété de cible existante de la ligne pour une réparation inchangée. Un nouvel identifiant Pool inconnu est rejeté ; le nonce renvoyé n'est jamais persisté. | 400 id/cible/nonce invalides ; 404 introuvable ; 409 modèle en double/ambigu ou conflit de configuration |
| `GET /api/codex-auth/account-target-options` | Renvoyer des choix de comptes avec métadonnées minimales et libellés respectueux de la confidentialité pour les éditeurs de modèles personnalisés ; aucune lecture de quota ni d’informations d’identification | — |
