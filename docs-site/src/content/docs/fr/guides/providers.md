...
**Le niveau sans clé `opencode-free` présente l'identité client anonyme qu'envoie la CLI OpenCode elle-même.** Lorsque le profil d'identité sans transfert s'applique, les requêtes portent un `x-opencode-session` dérivé de la conversation (ou limité à la requête si aucune identité de conversation n'est disponible), un User-Agent `opencode/<version>` versionné et le marqueur `x-opencode-client`. Le profil ajoute `Authorization: Bearer public` uniquement si aucun en-tête Authorization n'est déjà présent ; Zen l'associe à son contingent anonyme. Les requêtes avec `authMode: "forward"` ignorent ce profil. Provenance : `packages/opencode/src/session/llm/request.ts` (en-têtes) et `packages/console/app/src/routes/zen/util/handler.ts` (admission de la passerelle) d'OpenCode. Un `x-opencode-session` ou une clé d'API configurés priment toujours ; avec une clé, la requête est facturée sur ce compte. Les modèles contributeurs gratuits Muse Spark passent par le point de terminaison `/v1/responses` de Zen, conformément à la table des points de terminaison Zen. Cette admission est tolérée, pas contractuelle : OpenCode peut la modifier ou la restreindre à tout moment, auquel cas ces routes échouent avec l'erreur amont.
