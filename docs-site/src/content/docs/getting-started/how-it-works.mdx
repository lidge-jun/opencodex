...
- **Idle rolling windows can be activated on time.** Each account card offers default-off 5-hour
  ...
  its minimal non-stored account warmup request through the exact account whose window is due and
  records the reset timestamp to avoid ordinary restart duplicates. Paused accounts and accounts
