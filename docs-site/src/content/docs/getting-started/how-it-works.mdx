...
- **Idle rolling windows can be activated on time.** Each account card offers default-off 5-hour
  ...
  its minimal non-stored account warmup request, records the reset timestamp to avoid ordinary
  restart duplicates, and refreshes quota for the next schedule.
