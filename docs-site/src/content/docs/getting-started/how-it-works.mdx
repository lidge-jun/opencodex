...
- **New sessions can rebalance.** For a new thread, opencodex picks among eligible accounts using
  `accountPoolStrategy` (`quota` by default, or `round-robin` / `fill-first` / `reset-window`). The `quota` strategy
  compares known usage across 5h, weekly, and 30d windows and can switch to a lower-usage account
  when the active account crosses `autoSwitchThreshold`. `reset-window` instead orders fresh weekly
  reset timestamps (30-day for monthly plans), either nearest or farthest first; missing or stale reset evidence
  falls back to quota routing. Accounts in cooldown or needing
