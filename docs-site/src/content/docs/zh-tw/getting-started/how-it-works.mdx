...
2. **路由** —— `router.ts` 按固定的優先順序將請求的模型 id 對映到一個已設定的 provider：
   ...
   `defaultProvider` 回退。參見 [模型路由](/zh-cn/guides/model-routing/)。

3. **認證** —— 對於 `oauth` 型別的 provider,opencodex 會換入一個全新、自動重新整理的 access
   token 作為 bearer key,從而讓現有的 adapter 無需改動即可完成認證。

4. **Vision sidecar(可選)** —— 如果已路由的模型被列在 `provider.noVisionModels` 中,且
   請求攜帶了圖像，opencodex 會透過你的 ChatGPT 登入憑證，使用已設定的 vision sidecar 描述
   每張圖像並將其替換為文字，讓純文字模型仍可對圖像進行推理。
   參見 [Sidecar](/zh-cn/guides/sidecars/)。
...
6. **網路搜尋 sidecar(可選)** —— 如果 Codex 啟用了託管的 `web_search`,但已路由的模型
   ...
   agentic 迴圈中執行該模型，預設透過你的 ChatGPT 登入憑證呼叫 `gpt-5.6-luna` 執行真實搜尋，
   再將結果作為工具結果注入回去。
...
都能正確地往返。關於逐事件的對映,參見 [架構參考](/zh-cn/reference/architecture/)。
