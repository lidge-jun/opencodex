...
**Models** 頁面上的每個開關控制一個精確的模型 ID，作用於提供該模型的所有 provider（包括之後新增的）：關閉後該 ID 在所有地方隱藏。某個 provider 自己的隱藏會在該列以單獨的標籤顯示。依 provider 的可見性在 **Providers → Models** 中設定：**全部關閉** 隱藏該 provider 的所有模型，**全部開啟** 恢復它們並清除 allowlist，使之後新發現的模型也會開啟。任一層級的隱藏都會生效；恢復其中一層不會撤銷另一層。
