...
| `POST /api/codex-auth/reset-credits/consume` | 消耗 reset credit；需要重新輸入 owner admin token 驗證的 GUI session，或 CLI 的一次性本機同意 capability。單獨的管理認證或 `confirmed` 欄位不能取代。呼叫端必須持久重用同一 operation ID，直到收到 terminal code；quota refresh 是單獨的後續讀取 | 400 身分無效；403 `agent_consent_required`；上游狀態 passthrough；503 `server_busy`；500 消耗失敗 |
