...
檢查帳號的 Codex reset credits。消耗 credit 是破壞性操作，僅限使用者親自輸入並確認的執行同時提供 `--consume` 與 `--yes`。代理驅動的執行會在簽發一次性本機同意 capability 前遭拒；可重複使用的管理權杖不能取代該同意。CLI 會持久重用同一 operation ID，直到收到 terminal response；timeout 後請重新執行同一命令，不要開始新的消耗。
