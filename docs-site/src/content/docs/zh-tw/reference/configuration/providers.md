...
| `requestPacing?` | `{ enabled, requestsPerMinute?, minIntervalMs?, maxConcurrentRequests?, models? }` | 選用的用戶端出站請求啟動節流，與上游用量、計費及限流指標彼此獨立。RPM 會轉換為平均間隔；`minIntervalMs` 可施加更長的間隔。`maxConcurrentRequests` 限制同時進行中的傳送數，計數直到上游回應主體結束（從未讀取或取消的主體會在 30 秒後歸還租約，部分讀取後停擺的主體會在 300 秒不活動後歸還）；`models` 項目可設定更嚴格的每模型上限。`runTurn` 適配器以回合（而非每次實體傳送）持有一個租約：已持有回合內的後續傳送（例如 Cursor 重撥）僅按間隔准入。供應商限制適用於所有模型，`models` 依上游模型精確 ID 比對，只能增加延遲或收緊並發上限。排隊等待不計入回應標頭逾時。涵蓋 HTTP 與明確的適配器 `fetchResponse`/`runTurn` 呼叫；設定並發上限後，Responses WebSocket 回合會改走 HTTP/SSE 以便歸還租約。 |
