...
**Models** 页面上的每个开关控制一个精确的模型 ID，作用于提供该模型的所有 provider（包括之后添加的）：关闭后该 ID 在所有地方隐藏。某个 provider 自己的隐藏会在该行以单独的标签显示。按 provider 的可见性在 **Providers → Models** 中设置：**全部关闭** 隐藏该 provider 的所有模型，**全部开启** 恢复它们并清除 allowlist，使之后新发现的模型也会开启。任一层级的隐藏都会生效；恢复其中一层不会撤销另一层。
