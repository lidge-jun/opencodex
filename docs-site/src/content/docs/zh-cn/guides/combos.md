...
当前每个目标都不可用：例如，它的 provider 被禁用、它正在冷却、它已经在这次请求中被尝试过，或者加密的 v2 任务把它排除了。检查目标的 provider 状态和最近的上游错误。对于冷却，请等待配置的值或上游回退值（请求速率限制代码 `1302`/`1305` 为 5 秒，否则为 60 秒；永远不会超过 10 分钟），然后重试。
