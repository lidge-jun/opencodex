...
未设置 `cooldownMs` 时，发生跳转的目标使用上游回退值：对于上游代码为 `1302` 或 `1305` 的请求速率限制 429，等待 5 秒；账户配额窗口已耗尽时，等待 10 分钟；其他情况等待 60 秒。配额窗口耗尽通过上游代码（`usage_limit_exceeded`、`usage_limit_reached`、`1308`）或文本 `usage limit reached` 识别，与 HTTP 状态码无关：ChatGPT Codex 后端会把已耗尽的窗口报告为 502，而不是文档所述的 429。设置后，只要不存在可用的上游 `Retry-After` 或 Codex 重置信号，就会应用 `cooldownMs`，包括这些请求速率限制 429。接受数字形式的 `Retry-After` 秒数和 HTTP-date 值，显式上游 `Retry-After` 最多 24 小时；重置推导、配置和回退冷却最多 10 分钟。优先级从强到弱依次为：显式 `Retry-After` → Codex 重置标头（`x-codex-primary-reset-at`、`x-codex-secondary-reset-at` 或 `x-codex-tertiary-reset-at`）→ combo 的 `cooldownMs`（已设置时）→ 配额窗口耗尽时的 10 分钟冷却 → 上游速率限制代码 `1302`/`1305` 的 5 秒请求速率限制回退值 → 60 秒默认值。配额窗口会被优先判断，因此同时带有请求速率限制代码和用量上限文本的失败会冷却 10 分钟，而不是 5 秒。有效的即时指令 `Retry-After: 0` 会保留为上游即时指令，不会被配置的冷却替换。
...
| `cooldownMs` | 否 | 未设置 → 上游回退值（请求速率限制代码为 `1302`/`1305` 的 429 为 5 秒，配额窗口耗尽时为 10 分钟，否则为 60 秒） | 1 到 600000 的整数。设置后，只要没有可用的上游 `Retry-After` 或 Codex 重置信号，就会作为每个目标的冷却时间应用，包括请求速率限制 429；未设置时使用上游回退值。 |
...
当前每个目标都不可用：例如，它的 provider 被禁用、它正在冷却、它已经在这次请求中被尝试过，或者加密的 v2 任务把它排除了。检查目标的 provider 状态和最近的上游错误。对于冷却，请先遵循响应中的 `Retry-After` 值。Codex 重置标头的优先级也高于 `cooldownMs`；只有在两个上游信号都不可用时，才应用已配置的 `cooldownMs`，未配置时应用上游回退值（请求速率限制代码 `1302`/`1305` 为 5 秒，配额窗口耗尽时为 10 分钟，否则为 60 秒），显式 `Retry-After` 最多 24 小时，其他冷却最多 10 分钟。
