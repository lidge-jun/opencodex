...
| `POST /api/codex-auth/reset-credits/consume` | 消耗 reset credit；需要通过重新输入 owner admin token 验证的 GUI session，或 CLI 的一次性本地同意 capability。单独的管理认证或 `confirmed` 字段不能代替。调用方必须持久复用同一 operation ID，直到收到 terminal code；quota refresh 是单独的后续读取 | 400 身份无效；403 `agent_consent_required`；上游状态透传；503 `server_busy`；500 消耗失败 |
