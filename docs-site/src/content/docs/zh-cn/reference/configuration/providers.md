...
| `anthropicAccountPool.strategy?` | `"quota" \| "round-robin" \| "fill-first" \| "reset-window"` | `"quota"` | 新会话策略；`quota` 按 `quotaWindow` 指定的窗口对账户排序，`fill-first` 在同一窗口中判定耗尽阈值，`reset-window` 使用最新周重置时间。 |
