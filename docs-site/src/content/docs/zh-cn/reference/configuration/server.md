...
| `proxy?` | `string` | — | 出站 HTTP(S) 或 SOCKS 代理 URL（`socks5://host:port`），或 `${ENV_VAR}`。HTTP URL 仅在未设置时写入 `HTTP_PROXY` / `HTTPS_PROXY`。SOCKS URL 写入 `ALL_PROXY`（`ocx start --socks5`），并清除本进程继承的 `HTTP(S)_PROXY`。回环地址始终保留在 `NO_PROXY` 中。 |
