...
For explicit Windows x64 installation observation, see [the `attest` command](/reference/cli/agents/#explicit-installation-observation-on-windows-x64). It requires four supplied paths and does not grant update authority or attest runtime selection.
