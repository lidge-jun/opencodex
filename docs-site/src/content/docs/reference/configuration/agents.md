...
| `subagentModelFallbackUseSubagentModels?` | `boolean` | `false` | When enabled, encrypted v2 native-only fallback reuses the configured `subagentModels` roster as its ordered native fallback chain. It is opt-in because those models may use native ChatGPT credentials; an unset roster still produces no synthetic fallback. |
