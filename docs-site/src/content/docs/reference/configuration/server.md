...
| `allowRemoteDashboardSessions?` | `boolean` | `false` | Explicitly opt in to persistent dashboard sessions (`POST /api/auth/session`) for every non-loopback origin. The opted-in origin must also satisfy the HTTPS/management-allowlist checks, or the explicitly risky private/plain-HTTP policy. Plain HTTP carries the raw admin token on the first exchange; loopback keeps its short-lived injected session and does not need this flag. |
