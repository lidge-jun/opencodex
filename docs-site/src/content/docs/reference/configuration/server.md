...
| `codexDesktopAuthlessAuto?` | `boolean` | `false` | Opt-in automatic failover between authenticated and authless Desktop routing, driven by main-account Codex quota. While the 5h window reports exhausted, `codexDesktopAuthless` is engaged so independently credentialed routed models stay submittable in Codex Desktop; on recovery it is released again. Each transition rewrites `~/.codex/config.toml` and restarts Codex clients. `ocx system settings --desktop-authless-auto on`. |
