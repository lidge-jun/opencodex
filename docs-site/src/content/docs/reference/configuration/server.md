...
| `codexDesktopAuthless?` | `boolean` | `true` | Default-on authless Codex Desktop routing for standalone loopback targets without an admission token: inject the dedicated `opencodex` provider with `requires_openai_auth = false` so Desktop opens without a ChatGPT login. Ignored on non-loopback binds and admission-authenticated client targets. Toggle **Open Codex without signing in** in Dashboard → Overview, or use `ocx system settings --desktop-authless off` to opt out. Restart Codex after changing it. See [Codex integration](/guides/codex-integration/#authless-codex-desktop). |
