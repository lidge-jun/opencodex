...
| `codexDesktopAuthless?` | `boolean` | `true` | Default-on authless Codex Desktop routing on a loopback bind: inject the dedicated `opencodex` provider with `requires_openai_auth = false` so Desktop opens without a ChatGPT login. Ignored on non-loopback binds. Toggle **Open Codex without signing in** in Dashboard → Overview, or use `ocx system settings --desktop-authless off` to opt out. Restart Codex after changing it. See [Codex integration](/guides/codex-integration/#authless-codex-desktop). |
