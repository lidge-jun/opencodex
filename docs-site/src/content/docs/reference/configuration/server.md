...
| `codexQuotaMask?` | `boolean` | `false` | Keep the ChatGPT login usable in Codex Desktop after the account's usage window is exhausted: inject a root `chatgpt_base_url` pointing at the dedicated loopback listener, which relays `/backend-api/*` to `chatgpt.com` and masks only the `wham/usage` verdict so Desktop stops blocking every new chat. Requires `unauthenticatedLoopbackListener.enabled = true` with an explicit port; inert otherwise. Does not create quota — exhausted native-model calls still fail; routed external providers keep working. `ocx system settings --quota-mask on`. See [Codex integration](/guides/codex-integration/#keep-the-chatgpt-login-past-the-usage-wall-opt-in). |
