...
| `claudeCode.compatibility?` | `"shadow" \| "enforce"` | unset | Optional compatibility gate for routed Claude Messages translation only. Native Anthropic pass-through remains unchanged. `shadow` records bounded decisions in the persisted request log without rejecting; `enforce` rejects Anthropic features that cannot be represented losslessly by the selected routed adapter. Unset preserves legacy translation behavior. |
