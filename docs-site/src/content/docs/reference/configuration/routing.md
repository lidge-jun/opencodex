...
| `strategy?` | `"failover" \| "round-robin" \| "random" \| "least-used" \| "reset-window"` | `"failover"` | Selection strategy. Target order is failover priority and breaks ties. Weights shape round-robin and random selection. |
| `stickyLimit?` | `number` | `1` | Successful requests retained in one round-robin batch. Range 1–100; ignored by other strategies. |
...
`reset-window` prefers eligible targets whose earliest known quota reset is soonest. It reads only
the last successfully cached provider-quota snapshot, so request routing never waits for an upstream
quota probe. Missing or stale reset data ranks after known data; when every target is unknown, the
configured target order remains the fallback.
...
- A **combo** is explicit target routing and failover: configured order, weighted selection,
  successful-use history, or cached quota-reset timing decides, and failures advance through the list.
