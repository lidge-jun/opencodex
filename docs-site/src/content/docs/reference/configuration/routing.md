...
| `cooldownWaitPolicy?` | `"before-last-resort"` | unset | Defers targets marked `lastResort` while a normal target is cooling and that cooldown fits inside `waitForCooldownMs`. The deferral and the ordinary wait share one `waitForCooldownMs` budget per selection attempt. It never withholds the last resort when no normal target can be reached, and with `waitForCooldownMs` at its `0` default there is no wait to take, so nothing is deferred. Only that exact string opts in; `lastResort` is inert without it. |
