...
crossed. `codexAccountAutoSwitchThresholds` can replace that global threshold for the source account;
an absent entry inherits the global value and `0` disables only that account's usage-driven moves.
Pause, cooldown, reauthentication, and failure handling can clear or move routing
...
| Strategy | Behaviour |
| --- | --- |
| `quota` (default) | If no active account exists, choose the lowest-usage eligible account across 5-hour, weekly, and 30-day windows. Otherwise retain an eligible active account below its effective threshold (account override or global default); once it reaches that threshold, an unbound request or a bound task's next request can move to a lower-usage eligible account. `0` disables this usage-driven re-evaluation, not failure recovery. |
