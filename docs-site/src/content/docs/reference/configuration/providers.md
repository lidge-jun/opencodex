...
| `headers?` | `Record<string, string>` | Extra upstream headers. Authorization, cookies, API-key headers, embedded newlines, and invalid names are rejected. Proxy-originated requests — model discovery and connection tests — send `User-Agent: opencodex` unless a User-Agent is set here or by the provider preset. |
