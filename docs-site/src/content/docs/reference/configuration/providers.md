...
| `codexQuotaAutoRefresh?` | `Record<string, object>` | `{}` | Per-account opt-in for automatic `fiveHour` and `weekly` window activation. At a reported reset time, opencodex sends one minimal non-stored Codex message through that account, records the activated reset timestamp, and refreshes quota. The dashboard manages this field; internal `lastFiveHourResetAt` / `lastWeeklyResetAt` markers prevent normal service restarts from repeating the same window. |
