...
| `globalDisabledModelIds?` | `string[]` | — | Exact upstream model IDs hidden across every provider, including providers added later. The dashboard Models page edits this list; provider-specific hides in `disabledModels` remain independent. Direct requests remain available. |
