...
| `promptCacheKey?` | `boolean` | Provider-wide `openai-chat` opt-in for forwarding a caller-supplied `prompt_cache_key`; opencodex never generates the key. Default off. Enable only when the upstream documents support because strict gateways may reject the unknown field with HTTP 400. |
