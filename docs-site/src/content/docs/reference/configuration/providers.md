...
| `maxUpstreamBodyBytes?` | `number` | `15728640` (15 MiB) | Refuses a serialized native Responses passthrough body above this limit with local `413 outbound_body_too_large` before any upstream send. The error reports the measured size and identifies accumulated replayed `input_image` items when present; start a new session or compact the conversation to clear them. Fitting requests are unchanged. Set `0` to disable. |
