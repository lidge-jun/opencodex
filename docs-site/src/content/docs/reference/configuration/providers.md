...
| `apiKey?` | `string` | API key, an `${ENV_VAR}` / `$ENV_VAR` reference, or a `keychain:<provider>` reference written by `ocx provider keychain <name> store`. References resolve at request time. For `azure-openai`, omitting this field uses Microsoft Entra ID through `DefaultAzureCredential`. See [Storing keys in the OS keychain](#storing-keys-in-the-os-keychain). |
