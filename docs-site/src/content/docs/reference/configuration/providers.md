...
| `codexQuotaAutoRefresh?` | `Record<string, object>` | `{}` | Per-account opt-in for automatic `fiveHour` and `weekly` window activation. At a reported reset time, opencodex sends one minimal non-stored Codex message through that account and records the activated reset timestamp. The dashboard and account settings API manage this field through `/api/settings` without replacing unrelated settings; internal `lastFiveHourResetAt` / `lastWeeklyResetAt` markers prevent normal service restarts from repeating the same window. |
