...
SOCKS5 proxy uses the built-in tunnel, and an inherited SOCKS5 `ALL_PROXY` does so when `NO_PROXY`
does not match. Scheme-specific HTTP(S) proxy variables retain their separate native handling;
