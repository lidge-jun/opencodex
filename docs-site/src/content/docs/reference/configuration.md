...
| `codexAccountNamespaces?` | `Record<string,string>` | `{}` | Optional picker namespace to exact Codex account id map. Use `"main"` for the current Codex Desktop/main login. When non-empty, account-qualified native rows replace plain GPT rows in the picker and fail closed instead of switching accounts. Plain model ids remain routable. Namespace keys cannot collide with provider ids. |
