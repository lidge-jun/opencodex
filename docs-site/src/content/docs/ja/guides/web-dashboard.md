...
**モデル** ページの各スイッチは、完全一致するモデル ID を、それを提供するすべてのプロバイダー（後から追加したものを含む）でまとめて制御します。オフにするとその ID はどこでも非表示になります。プロバイダー単位の非表示は、行に別のチップとして表示されます。プロバイダー単位の表示は **プロバイダー → モデル** で設定し、**すべてオフ** はそのプロバイダーの全モデルを非表示に、**すべてオン** は再表示して allowlist を解除し、新しく見つかったモデルもオンにします。どちらの非表示も優先され、一方を戻してももう一方は解除されません。
