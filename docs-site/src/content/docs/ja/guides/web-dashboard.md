...
| **ログ** | カーソルページングされた永続リクエストメタデータを、プロバイダー、モデル、状態、プロトコル、キー、プロファイル、フォールバックでサーバー側フィルターできます。詳細は記録済みのルート決定、試行列、最終ターゲットと結果を読み込み、トレース以前の行は不明のまま表示します。 |
...
| `GET /api/request-history?limit=200&cursor=...` · `GET /api/request-history/:requestId/route-decision` | カーソルページングされた永続履歴と、記録済みの why-this-route 証拠を読み取ります。旧 `/api/logs` リングは互換性のため残ります。 |
