...
プロバイダーデバッグを有効にすると、互換性のための制約緩和について、エンドポイント
クラス、固定の損失カテゴリ、上限付き件数、切り詰めフラグだけを出力します。ツール名、
プロパティ名、パス、値、スキーマ本文は含まれません。この診断は既存の互換変換を観測
するだけで、要求を拒否しません。ネイティブ出力スキーマは診断対象外です。
