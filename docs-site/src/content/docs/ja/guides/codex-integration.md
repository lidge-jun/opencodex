...
メインアカウントのデバイスコードによる再認証をキャンセルする際、HTTP 404 と `unknown_flow` コードが返された場合は期限切れのフローを解放して再ログインを開始できるようにしますが、ログイン成功やキャンセル確定とは表示しません。 キャンセルが確認されると、デバイスコードによる再ログインを開始できます。以前のフローに対する開始・状態確認・キャンセルの遅延応答は、新しいフローを変更せず、ログイン成功も通知しません。
