...
応答は生成された `opencodex-catalog.json` ドキュメントです。データプレーンのルートは現行のカタログ配布セーフティ チェックを適用し、資格情報・アイデンティティ・設定の形として認識できる内容を拒否します。実施方式はメンテナーのレビュー中です。利用可能な場合、`x-opencodex-codex-version` ヘッダーはサーバー上の Codex ランタイム バージョンを報告するため、クライアントはバージョンの偏りを特定できます。
