...
- **xAI Imagine (Grok OAuth) リレー:** `images.bridgeEnabled` が `true` で、`xai` プロバイダーに利用可能な Grok CLI OAuth トークン（`ocx login xai`）または API キーがある場合、`/v1/images/generations` と `/v1/images/edits` はそのトークン付きで `https://api.x.ai/v1` に送られます。ChatGPT の資格情報は転送されません。リレーは Codex の `size` / `aspect_ratio` を xAI Imagine のボディに写し、同じ `{created, data:[{b64_json}]}` 形を返します。バッチ全体（インライン `b64_json` とダウンロードした URL）のデコード済みバイトと base64 エンコード出力は合わせて 100 MiB 未満です。上限を超えるバッチは 502 を返します。これは API キー専用の Responses Image Bridge ループとは独立です。
