...
フォールバックでは、互換性のない暗号化タスクは読み取り可能になりません。子タスクが ChatGPT 用に暗号化されている場合、選択は正規のネイティブ ChatGPT ターゲット、または `allowEncryptedV2AgentTasks: true` が明示された Responses プロバイダーに制限されます。

## 暗号化された v2 タスク配信

Codex は、v2 ネイティブからルーティングされた子タスクを、バックエンドで暗号化された `encrypted_content` としてのみ送信できます。ネイティブ ChatGPT バックエンドは処理でき、一部の互換リレーも処理可能なバックエンドへ転送できますが、OpenCodex は名前や Base URL からこの能力を推測できません。これは既知の [#92限定](https://github.com/lidge-jun/opencodex/issues/92) です。

opencodex は、空のタスクまたは読み取り不可能なタスクを転送するのではなく、安全に失敗します。

- 明示的に信頼されていない直接の非ネイティブ ルートは HTTP 400 を返します。
`error.code = "unreadable_encrypted_agent_task"` であり、暗号文はエコーされません。
- コンボでは、再試行を含む、そのタスクの正規のネイティブ ChatGPT ターゲットと明示的に信頼された Responses ターゲットのみが考慮されます。何もない場合
...
非標準 Responses エンドポイントがこの暗号文を処理できると確認した場合、プロバイダー設定で **暗号化された V2 エージェントタスクを転送** を有効にするか、`allowEncryptedV2AgentTasks: true` を設定します。既定では無効で、`adapter: "openai-responses"` でのみ有効です。不透明なタスクを変更せずに転送するだけで、復号、平文復元、互換性の証明は行わず、このルートでは `agentTaskRecovery` をスキップします。
