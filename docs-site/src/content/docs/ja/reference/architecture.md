...
2. `server/responses/core.ts` が圧縮を解除して JSON を解析します。覚えておいた `previous_response_id` 入力があれば展開します（完全な履歴の再送はそのまま保持し、保存済み履歴を再度前置しません）。その後 `responses/parser.ts` に渡します。ルーティング先モデルの実効入力上限を超えると推定される入力は、上流 I/O の前に `413 request_too_large` で拒否されます。
