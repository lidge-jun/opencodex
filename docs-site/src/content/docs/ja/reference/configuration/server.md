...
| `proxy?` | `string` | — |送信 HTTP(S) または SOCKS プロキシ URL（`socks5://host:port`）または `${ENV_VAR}`。HTTP URL は未設定時のみ `HTTP_PROXY` / `HTTPS_PROXY` に適用されます。SOCKS URL は `ALL_PROXY` に適用され（`ocx start --socks5`）、このプロセスで継承した `HTTP(S)_PROXY` をクリアします。ループバックは `NO_PROXY` に残ります。 |
