...
専用の 402/429 障害ローテーションは、通常の Responses メイン送信パスとターミナル継続パスに限定されます。
Google にルーティングされる場合、`image/video bridge` と `web-search sidecar` のループはリクエストで
最初に選択されたアカウントを使用できますが、この専用プールを通じてそのアカウントをクールダウンまたは
ローテーションしません。スタンドアロンの Antigravity 画像エンドポイントも、このローテーションパスの対象外です。
...
プロバイダー固有のプールが有効でない間、OAuth プロバイダーのアカウントがレート制限を受けると、
同じプロバイダーの別のログイン済みアカウントへローテーションします。対象は xAI、Cursor、Kimi、
GitHub Copilot、Google Antigravity、Nous です。`googleAntigravityAccountPool.enabled` が `true` の
場合は、その専用プールが代わりに Google のルーティングを専有します。無効の場合でも、この汎用
ポリシーによって緊急時の 429 フェイルオーバーが行われることがあります。

**2 つ目のアカウントへのログインが、この機能を有効にする契機です。** 設定がない場合、対象
プロバイダーに再認証が必要とされていないアカウントが 2 つ以上あるとローテーションが有効になります。
これは `apiKeyPool` が 2 つ以上のキーを持つプールにすでに適用しているものと同じルールです。保存済み
アカウントが 1 つのプロバイダーは、従来とまったく同じように動作します。

| キー | 型 | デフォルト | 説明 |
| --- | --- | --- | --- |
| `oauthAccountFailover.enabled?` | `boolean` | アカウントの存在に応じる | グローバルなオーバーライドです。`false` はすべてのプロバイダーを単一アカウント動作に固定し、`true` はローテーションを強制的に有効にします。 |
| `providers.<name>.oauthAccountFailover.enabled?` | `boolean` | 継承 | プロバイダー単位のオーバーライドです。グローバル設定とアカウントの存在より優先されます。 |

利用規約上のリスクを避けたい特定のプロバイダーを厳密な単一アカウント動作に保つには、次のようにします。
...
この設定は、ログイン、アカウントの追加、再認証を行っても保持されます。

この機能は意図的に `anthropicAccountPool` より限定されています。セッション親和性、クォータ順位による
選択、プローブリースはありません。答えるのは 1 つの問いだけです。つまり、429 を返したばかりの
アカウントがクールダウン中のとき、利用できる別のアカウントがあるかどうかです。

Codex プールと Anthropic プールは対象外であり、それぞれ独自のローテーションを維持します。この機能を
有効にしても、どちらにも変更はありません。保存済みアカウントが 1 つだけのプロバイダーでは何も行われず、
そのアカウントにクールダウンは記録されません。

429 が発生すると、失敗したアカウントは `Retry-After` がある場合はその値（上限 15 分）を使い、ない場合は
デフォルトのバックオフでクールダウンされます。その後、次の適格なアカウントでリクエストが再実行され、
1 リクエストにつき最大 3 回ローテーションします。再認証が必要とマークされたアカウントは選択されません。
クールダウンはプロセスローカルであるため、再起動すると失われます。

ローテーションでは、代替アカウントのベアラーだけでなく、認証情報の**完全な**スナップショットを引き継ぎます。
そのため、トークンとルーティングメタデータを対応付けるプロバイダー（たとえば Antigravity の Cloud Code Assist
プロジェクト ID）で、あるアカウントのトークンを別のアカウントのメタデータと組み合わせて送信することはありません。

現在の対象範囲は通常の Responses リクエストパスです。Cursor はレート制限を HTTP ステータスではなく
アダプターイベントとして報告し、スタンドアロンの Antigravity 画像エンドポイントには独自のリクエストパスが
あります。現時点では、どちらもローテーションしません。

:::caution[実験的]
サブスクリプションアカウント間のローテーションは 2 つ目のアカウントのクォータを消費し、一部のプロバイダーの
利用規約に違反する可能性があります。このトレードオフを望まない場合は、グローバルまたは該当プロバイダーに対して
