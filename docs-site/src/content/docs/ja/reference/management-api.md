...
|メソッドとパス |目的 |注目すべきエラー |
| --- | --- | --- |
| `GET /api/catalog` |インストールされている Codex カタログ ドキュメントを返します | 404 カタログが見つかりません。500 配布してはならない値がカタログに含まれています。500 カタログ ファイルが大きすぎて読み取れません |
...
`GET /api/catalog` はダッシュボードと運用ツール向けです。リモート クライアントがカタログを必要とする
場合は、データプレーンのルート `GET /v1/catalog` を使用してください。同じ生成器から同じドキュメントを
返すため、クライアント マシンが管理シークレットを受け取ることはありません。
[Proxy API Formats](/ja/reference/proxy-formats/)を参照してください。

2 つのルートは同一のマテリアライズ処理を共有するため、このルートはその 2 つの拒否を引き継ぎます。
いずれも通常の管理エラー エンベロープ `{ "error": "<message>" }` で返り、データプレーンの
`type`/`code` エンベロープではありません。したがって `/v1/catalog` のエラー コードはここには現れません。

|ステータス |レスポンス本文 |意味 |
| --- | --- | --- |
| 404 | `{ "error": "catalog not found" }` |カタログ ドキュメントを読み取れませんでした |
| 500 | `{ "error": "catalog contains values that are not safe to distribute" }` |保存されたカタログに資格情報・アイデンティティ・設定の形をした内容が含まれています。このメッセージは意図的に内容を含みません |
| 500 | `{ "error": "catalog file is too large to read" }` |保存されたファイルが安全な 32 MiB のソース読み取り上限を超えたため、解析前に拒否されました |

このルートはデータプレーンの 8 MiB シリアライズ応答上限を適用**しません**。その境界はリモートから
到達可能なトランスポートにのみ属します。

:::note
共有された拒否が管理ルートにも適用されるのは、マテリアライズ処理が 1 つであることの帰結であり、
承認された方針ではありません。以前このルートは解析可能なカタログ ファイルをすべて提供していました。
管理プレーンが同じ拒否の対象であり続けるべきかは、`structure/05_gui-and-management-api.md` に
記録された未決のメンテナー判断です。
:::
