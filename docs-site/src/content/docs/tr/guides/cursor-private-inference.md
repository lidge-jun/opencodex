...
- **Kurulu derlemeler.** Cursor Private Inference'ın (yol ve sürümle) ve normal Cursor'ın (yalnızca yolla) bulunup bulunmadığını gösterir. Yalnızca normal Cursor bulunursa sekme, Cursor'ın kendi güncelleme kanalına (`api2.cursor.sh` üzerindeki `cursor-local` kanalı) bu platform ve işlemci için hangi local-mode yükleyicisini sunduğunu sorar, o sürümü `downloads.cursor.com/local-mode/` yükleyicisine giden bir bağlantıyla gösterir ve buraya bağlantı verir. opencodex yalnızca bağlantıyı gösterir; hiçbir şeyi indirmez, başlatmaz veya kurmaz. Yanıt 30 dakika (hatadan sonra 5 dakika) önbellekte tutulur. Kanala ulaşılamazsa, kullanılamaz bir yanıt gelirse ya da Cursor bu bilgisayar için bir derleme sunmuyorsa (yalnızca Windows, macOS ve Linux'ta x64 ile arm64 için var) sekme, yükleyicinin belirlenemediğini söyler; aşağıdaki ağ geçidi değerleri her durumda geçerlidir. Normal Cursor'ın kendisi özel uç noktaları hâlâ Cursor sunucuları üzerinden yönlendirdiği için geri döngü proxy'sine erişemez.
