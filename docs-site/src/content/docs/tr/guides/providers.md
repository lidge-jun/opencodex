...
Antigravity kotası, CCA'nın `retrieveUserQuota` ve `retrieveUserQuotaSummary` RPC'leriyle (8 saniyelik zaman aşımı) canlı olarak sorgulanır; canlı kota kullanılamadığında sürdürülen katalog geri dönüş olur.
