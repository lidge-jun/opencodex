...
| `reset-window` | Uygun hesapları geçerli gelecekteki governing reset zamanına göre sıralar. `accountPoolResetOrder`, en yakını (`soonest`) veya en uzağı (`latest`) önce seçer. Başka bir hesapta pay varken eşikteki hesap atlanır; kanıt eksik ya da eskiyse `quota` kullanılır. Sağlıklı bağlı görevler bağlılığı korur. |
...
| `anthropicAccountPool.strategy?` | `"quota" \| "round-robin" \| "fill-first" \| "reset-window"` | `"quota"` | Yeni oturum stratejisi; `quota` hesapları `quotaWindow` penceresine göre sıralar, `fill-first` tükenme eşiğini aynı pencerede değerlendirir ve `reset-window` geçerli gelecekteki haftalık sıfırlamayı kullanıp kanıt eksik ya da eskiyse quota seçimine döner. |
...
| `anthropicAccountPool.resetOrder?` | `"soonest" \| "latest"` | `"soonest"` | En yakın veya en uzak haftalık sıfırlamayı önce seçer. |
