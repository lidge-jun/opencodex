...
| `autoSwitchThreshold?` | `number` | `80` | Proaktif geçiş için genel varsayılan kullanım eşiği; `codexAccountAutoSwitchThresholds` bunu hesap bazında geçersiz kılar. `quota`, bağlı ve bağımsız görevleri mevcut kaynak hesabın etkin eşiğine göre yeniden değerlendirir; `fill-first`, bağımsız atama için her hesabın kota payını kendi etkin eşiğine göre kontrol eder. `round-robin` dönüşü sayaç tabanlıdır ve eşiği kullanmaz, ancak ortak öncelik katmanı filtresi her hesabın etkin eşiğini kullanmaya devam eder. Puan, bilinen 5 saatlik, haftalık veya 30 günlük kota pencerelerindeki en yüksek kullanımı alır. Etkin değer `0` yalnızca bu hesaptan kullanıma dayalı proaktif geçişi devre dışı bırakır; bağımsız atamayı veya arıza kurtarmayı devre dışı bırakmaz. |
