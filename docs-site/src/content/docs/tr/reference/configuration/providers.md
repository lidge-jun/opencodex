...
| `anthropicAccountPool.strategy?` | `"quota" \| "round-robin" \| "fill-first" \| "reset-window"` | `"quota"` | Yeni oturum stratejisi; `quota` hesapları `quotaWindow` penceresine göre sıralar, `fill-first` tükenme eşiğini aynı pencerede değerlendirir ve `reset-window` güncel haftalık sıfırlamayı kullanır. |
