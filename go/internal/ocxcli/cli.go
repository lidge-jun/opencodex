...
var Commands = []Command{
	...
	// update and v2 stay TypeScript-owned (issue #54 remainder, recorded):
	// `ocx update` performs a network release fetch + in-place self-replace, so
	// no hermetic byte-diff oracle can drive it; `ocx v2` reads AND writes the
	// upstream Codex config.toml (feature toggles, threads, subagent
	...
	// claude and opencode stay TypeScript-owned (issue #54 remainder, recorded):
	// both are launchers, but unlike mcode/mmx their launch env is assembled by
	// whole auth/credential subsystems - claude resolves subscription vs
	// proxy-auth mode (src/claude/auth-detect + auth-mode, launcher-context
	// parent-env provenance, gateway-cache refresh, agents-inject writes under
	// ~/.claude) and opencode merges V1/V2 runtime provider blocks into a
	// project JSONC config with a live model catalog. A byte-faithful port
	// would need every one of those subsystems plus a live-server oracle; an
	// approximate env assembly would silently break real subscription auth,
