...
// cliUsageFailure mirrors runCliAction's CliUsageError lane (exit 2 with the
...
func (f *cliUsageFailure) Error() string { return f.message }
...
func takeRuntimeOption(args *[]string, flag string) (string, error) {
	for index, arg := range *args {
		...
		if index+1 >= len(*args) || strings.HasPrefix((*args)[index+1], "--") {
			return "", &cliUsageFailure{message: flag + " requires a value"}
		}
		value := (*args)[index+1]
...
func rejectRuntimeArgs(args []string, usage string) error {
	if len(args) > 0 {
		return &cliUsageFailure{message: "Unexpected argument(s): " + strings.Join(args, " "), usage: usage}
	}
	return nil
}
...
func clientIntegrationRows(value *jsonwire.Value) []string {
	...
	for _, row := range member.Elements() {
		...
		idText := fieldString(row, "clientId")
		stateText := fieldString(row, "state")
...
func clientIntegrationOperations(value *jsonwire.Value) []string {
	...
	for _, row := range elements {
		...
		at := fieldString(row, "at")
		clientID := fieldString(row, "clientId")
		kind := fieldString(row, "kind")
		backup := "op " + fieldString(row, "opId")
...
func runClientIntegrationCommand(argv []string, deps Deps) int {
	...
	case "restore":
		...
		if opID == "" {
			return runCliActionResult(&cliUsageFailure{message: "--op <opId> is required", usage: clientIntegrationUsage})
		}
	...
	case "enable", "disable":
		...
		if client == "" {
			return runCliActionResult(&cliUsageFailure{message: "--client <id> is required", usage: clientIntegrationUsage})
		}
		if overwriteConflict && action == "disable" {
			return runCliActionResult(&cliUsageFailure{message: "--overwrite-conflict applies only to enable", usage: clientIntegrationUsage})
		}
		var body []byte
		...
		return ExitOK
	default:
		return runCliActionResult(&cliUsageFailure{
