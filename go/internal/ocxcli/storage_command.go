...
func storageCleanup(argv []string, deps Deps) error {
	...
	preview, _, requestErr := managementRequest(deps, "POST", "/api/storage/cleanup/preview", previewBody)
	if requestErr != nil {
		return requestErr
	}

	if !confirmed {
		printManagementData(deps, preview, wantsJSON, storagePreviewLines(preview))
		return nil
	}
	...
	result, _, requestErr := managementRequest(deps, "POST", "/api/storage/cleanup", body)
	if requestErr != nil {
		return requestErr
	}
...
func storageTrash(argv []string, deps Deps) error {
	...
	if action == "list" {
		...
		result, _, requestErr := managementRequest(deps, "GET", "/api/storage/trash", nil)
		if requestErr != nil {
			return requestErr
		}
		printManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
	...
	result, _, requestErr := managementRequest(deps, "POST", "/api/storage/trash/restore", body)
	if requestErr != nil {
		return requestErr
	}
	printManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
...
func storagePolicy(argv []string, deps Deps) error {
	...
	if action == "show" {
		...
		result, _, requestErr := managementRequest(deps, "GET", "/api/storage/cleanup-policy", nil)
		if requestErr != nil {
			return requestErr
		}
		printManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
	...
	if action == "set" {
		...
		result, _, requestErr := managementRequest(deps, "PUT", "/api/storage/cleanup-policy", body)
		if requestErr != nil {
			return requestErr
		}
		printManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
	...
	result, _, requestErr := managementRequest(deps, "POST", "/api/storage/cleanup-policy/run", nil)
	if requestErr != nil {
		return requestErr
	}
	printManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
...
func runStorage(args []string, deps Deps) int {
	return runManagementAction(deps, func() error {
		...
		case "report":
			...
			result, _, requestErr := managementRequest(deps, "GET", "/api/storage", nil)
			if requestErr != nil {
				return requestErr
			}
			printManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
