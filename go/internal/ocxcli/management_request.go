...
// familyEncodeURIComponent mirrors familyEncodeURIComponent: every byte except the
// unreserved set is percent-encoded with uppercase hex.
func familyEncodeURIComponent(value string) string {
...
// familyManagementRequest performs one management-plane request exactly like
...
func familyManagementRequest(deps Deps, method, path string, body *jsonwire.Value) (*jsonwire.Value, int, error) {
	deps = defaults(deps)
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		return nil, response.StatusCode, newAPIError(familyManagementResponseMessage(value, response.StatusCode), response.StatusCode)
	}
	return value, response.StatusCode, nil
}

// familyManagementResponseMessage mirrors responseMessage: a non-empty string body
// wins (truncated to 400), otherwise the error/message/detail field with
// reason/hint appended; the generic line is the fallback.
func familyManagementResponseMessage(body *jsonwire.Value, status int) string {
...
func runManagementAction(deps Deps, action func() error) int {
	if err := action(); err != nil {
		...
		var apiErr *familyManagementAPIError
		switch {
...
func errorsAs(err error, target any) bool {
	...
	case **familyManagementAPIError:
		if e, ok := err.(*familyManagementAPIError); ok {
			*typed = e
			return true
		}
	}
...
// familyPrintManagementData mirrors printData: JSON.stringify(v, null, 2) when JSON
// output was requested (or no human lines exist), otherwise one console.log
// per line.
func familyPrintManagementData(deps Deps, value *jsonwire.Value, wantsJSON bool, lines []string) {
