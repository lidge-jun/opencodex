...
func agentStatus(deps Deps, argv []string) error {
	...
	for _, item := range []struct{ key, path string }{
		...
		value, _, requestErr := managementRequest(deps, "GET", item.path, nil)
		if requestErr != nil {
			return requestErr
		}
		merged.Set(item.key, value)
	}
	printManagementData(deps, merged, wantsJSON, managementSummaryLines(merged, "", 0))
	return nil
}
...
func agentInjection(deps Deps, argv []string) error {
	...
	if action == "status" {
		...
		result, _, requestErr := managementRequest(deps, "GET", "/api/injection-model", nil)
		if requestErr != nil {
			return requestErr
		}
	...
	result, _, requestErr := managementRequest(deps, "PUT", "/api/injection-model", body)
	if requestErr != nil {
		return requestErr
	}
	printManagementData(deps, result, wantsJSON, []string{"Agent injection settings updated."})
	return nil
}
...
func agentEffort(deps Deps, argv []string) error {
	...
	if action == "status" {
		...
		result, _, requestErr := managementRequest(deps, "GET", "/api/effort-caps", nil)
		if requestErr != nil {
			return requestErr
		}
		printManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
	...
	result, _, requestErr := managementRequest(deps, "PUT", "/api/effort-caps", body)
	if requestErr != nil {
		return requestErr
	}
	printManagementData(deps, result, wantsJSON, []string{"Agent effort caps updated."})
	return nil
}
