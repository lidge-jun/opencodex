...
// familyManagementAPIError mirrors RuntimeApiError's observable shape: a message and
// an HTTP status that selects the exit code.
type familyManagementAPIError struct {
	message string
	status  int
}

func (e *familyManagementAPIError) Error() string { return e.message }
...
func newAPIError(message string, status int) error {
	return &familyManagementAPIError{message: message, status: status}
}
