...
func runNativeIntegration(args []string, deps Deps) int {
	return runManagementAction(deps, func() error {
		...
		if action == "list" {
			...
			result, _, requestErr := managementRequest(deps, "GET", "/api/native-integrations", nil)
			if requestErr != nil {
				return requestErr
			}
			printManagementData(deps, result, wantsJSON, nativeLines(result))
			return nil
		}
		known := false
		...
		result, _, requestErr := managementRequest(deps, "PUT", "/api/native-integrations/"+action, body)
		if requestErr != nil {
			return requestErr
		}
