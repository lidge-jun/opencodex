...
func runClientIntegration(args []string, deps Deps) int {
	return runManagementAction(deps, func() error {
		...
		case "status", "show", "list":
			...
			if clientGiven {
				path = "/api/client-integrations/" + familyEncodeURIComponent(client)
			}
			...
			familyPrintManagementData(deps, result, wantsJSON, lines)
			return nil
		...
		case "history", "journal":
			...
			if clientGiven {
				path += "?client=" + familyEncodeURIComponent(client)
			}
			result, _, requestErr := familyManagementRequest(deps, "GET", path, nil)
			if requestErr != nil {
				return requestErr
		...
		case "restore":
			...
			result, _, requestErr := familyManagementRequest(deps, "POST", "/api/client-integrations/restore", body)
			if requestErr != nil {
				return requestErr
			}
			familyPrintManagementData(deps, result, wantsJSON, []string{clientMessageField(result.Find("message"), "Restored.")})
			return nil
		}
		...
		familyPrintManagementData(deps, result, wantsJSON, []string{clientMessageField(result.Find("message"), message)})
		return nil
	})
}
