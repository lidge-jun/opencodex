...
func runClaudeConfig(args []string, deps Deps) int {
	return runManagementAction(deps, func() error {
		...
		if action == "status" || action == "show" {
			...
			result, _, requestErr := managementRequest(deps, "GET", "/api/claude-code", nil)
			if requestErr != nil {
				return requestErr
			}
		...
		result, _, requestErr := managementRequest(deps, "PUT", "/api/claude-code", body)
		if requestErr != nil {
			return requestErr
		}
		printManagementData(deps, result, wantsJSON, []string{"Claude Code settings updated."})
		return nil
	})
}
