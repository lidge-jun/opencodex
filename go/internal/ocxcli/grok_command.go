...
func grokStateExcluded(deps Deps) ([]string, error) {
	state, _, requestErr := familyManagementRequest(deps, "GET", "/api/grok", nil)
	if requestErr != nil {
...
func runGrok(args []string, deps Deps) int {
	return runManagementAction(deps, func() error {
		...
		if action == "status" || action == "show" {
			...
			result, _, requestErr := familyManagementRequest(deps, "GET", "/api/grok", nil)
			if requestErr != nil {
				return requestErr
			}
		...
		if action == "apply" {
			...
			result, _, requestErr := familyManagementRequest(deps, "POST", "/api/grok/apply", nil)
			if requestErr != nil {
				return requestErr
			}
			...
			familyPrintManagementData(deps, result, wantsJSON, []string{message})
			return nil
		}
		var excluded []string
		switch action {
		...
		result, _, requestErr := familyManagementRequest(deps, "PUT", "/api/grok/selection", body)
		if requestErr != nil {
			return requestErr
		}
		...
		familyPrintManagementData(deps, result, wantsJSON, []string{"Grok exclusions: " + message})
		return nil
	})
}
