...
func agentSubagents(deps Deps, argv []string) error {
	...
	if action == "status" {
		...
		result, _, requestErr := familyManagementRequest(deps, "GET", "/api/subagent-models", nil)
		if requestErr != nil {
			return requestErr
		}
	...
	result, _, requestErr := familyManagementRequest(deps, "PUT", "/api/subagent-models", body)
	if requestErr != nil {
		return requestErr
	}
	...
	familyPrintManagementData(deps, result, wantsJSON, []string{"Subagent roster: " + message})
	return nil
}
...
func agentFallback(deps Deps, argv []string) error {
	...
	if action == "status" {
		...
		result, _, requestErr := familyManagementRequest(deps, "GET", "/api/subagent-model-fallback", nil)
		if requestErr != nil {
			return requestErr
		}
		familyPrintManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
	...
	result, _, requestErr := familyManagementRequest(deps, "PUT", "/api/subagent-model-fallback", body)
	if requestErr != nil {
		return requestErr
	}
	familyPrintManagementData(deps, result, wantsJSON, []string{"Subagent fallback settings updated."})
	return nil
}
...
func agentReadSidecarSettings(deps Deps) (*jsonwire.Value, error) {
	value, _, requestErr := familyManagementRequest(deps, "GET", "/api/sidecar-settings", nil)
...
func agentSidecar(deps Deps, argv []string) error {
	...
	if section == "status" {
		...
		result, _, requestErr := familyManagementRequest(deps, "GET", "/api/sidecar-settings", nil)
		if requestErr != nil {
			return requestErr
		}
		familyPrintManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
	...
	if wantsList {
		...
		if section == "web" {
			...
			familyPrintManagementData(deps, optionsValue, wantsJSON, lines)
		} else {
	...
	result, _, requestErr := familyManagementRequest(deps, "PUT", "/api/sidecar-settings", body)
	if requestErr != nil {
		return requestErr
	}
	familyPrintManagementData(deps, result, wantsJSON, []string{fmt.Sprintf("%s sidecar settings updated.", section)})
	return nil
}
...
func agentRequestUserInput(deps Deps, argv []string) error {
	...
	if state == "" {
		result, _, requestErr := familyManagementRequest(deps, "GET", path, nil)
		if requestErr != nil {
			return requestErr
		}
		familyPrintManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
	...
	result, _, requestErr := familyManagementRequest(deps, "PUT", path, body)
	if requestErr != nil {
		return requestErr
	}
	familyPrintManagementData(deps, result, wantsJSON, managementSummaryLines(result, "", 0))
